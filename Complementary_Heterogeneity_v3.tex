...
\subsection{Structural Estimation of $\rho$}\label{sec:structural_rho_v3}

The curvature parameter $\rho$ is adopted at each hierarchy level from published micro-data production function estimates whose aggregation concept is closest to the thesis level, and cross-validated with NLS estimates from FRED Industrial Production data (monthly, 1972--2025, $T = 649$; moving-block bootstrap, $B = 1{,}000$).

\medskip
\noindent
\begin{tabular}{@{}clllcc@{}}
\toprule
Level & Concept & Source & $\sigma$ range & $\rho$ range & $\hat\rho_{\text{NLS}}$ \\
\midrule
1 & Hardware (semiconductors) & Oberfield \& Raval (2021) & $[0.5, 0.7]$ & $[-1.0, -0.4]$ & $-0.19$ \\
2 & Mesh (AI agents) & Atalay (2017) & $[0.1, 0.5]$ & $[-9.0, -1.0]$ & $-0.30$ \\
3 & Training (capability) & Oberfield \& Raval + Gechert (2022) & $[0.5, 0.8]$ & $[-1.0, -0.2]$ & $0.15$ \\
4 & Settlement (stablecoins) & Broda \& Weinstein (2006) & $[2.0, 5.0]$ & $[0.5, 0.8]$ & $0.67$ \\
\bottomrule
\end{tabular}

\medskip
\noindent NLS estimates are systematically above the adopted micro-data ranges (3 of 4 levels), consistent with FRED IP indices aggregating more broadly than plant-level production functions.  The ordering is preserved across methods.  The overidentification test ($\chi^2 = 6.37$, $df = 5$, $p = 0.27$) cannot reject the structural interpretation.
