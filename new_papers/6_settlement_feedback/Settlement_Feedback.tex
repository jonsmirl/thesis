...
\caption{\textbf{USMPD high-frequency decomposition.} (a)~Statement concentration ratio (30-minute yield change as fraction of daily move) over rolling 3-year windows, showing declining concentration ($\tau = -0.172$, $p < 0.001$). (b)~Surprise-normalized 10-year yield response per unit of monetary policy surprise, showing no trend ($\tau = +0.015$, $p = 0.717$). (c)~Cross-maturity ratio $|\Delta\text{UST2Y}|/|\Delta\text{UST10Y}|$, stable. (d)~Absolute 30-minute response magnitude, declining but explained by less news per statement rather than FG degradation. Data: SF Fed USMPD (Acosta et al.\ 2025), FRED DGS2/DGS10.}
\label{fig:usmpd}
\end{figure}
...
\caption{\textbf{Composite monetary policy degradation.} (a)~FOMC importance ratio over rolling 3-year windows for DGS2 and DGS10. (b)~QE multiplier (bp per \$100B WALCL expansion) declining monotonically across four episodes: QE1 (40.8), QE2 (9.7), QE3 (3.4), COVID-QE (2.7). (c)~Financial repression: rolling 60-month correlation between real 10-year yield and personal savings rate, showing the FR mechanism remains intact. (d)~Normalized $[0,1]$ degradation indices: the ordering QE (0.93) $>$ FR (0.57) $>$ FG (0.31) reflects pre-mesh technology that addresses arbitrage speed but not language processing or institutional barriers.}
...
The model generates seven predictions extending the prediction sets of the companion papers. Each specifies timing, observable metrics, and conditions under which the prediction would be falsified.
...
\textbf{Prediction 7: CBDC self-undermining.} Among countries that launch retail CBDCs, the displacement ratio $\theta_{\text{displace}}/\theta_{\text{restrict}}$ (offshore settlement gained per unit of onshore settlement suppressed) exceeds 0.5 within 3 years of launch. The observable metric is the growth rate of unsurveilled digital settlement volume (stablecoins, privacy protocols, foreign CBDCs) in the launching country relative to a synthetic control of comparable non-CBDC countries. The India natural experiment (Section~6.3) provides the calibration: $\theta_{\text{displace}}/\theta_{\text{restrict}} \approx 0.84$ for cryptocurrency restrictions. The CBDC prediction is weaker (0.5 vs.\ 0.84) because the CBDC provides a partial substitute for private stablecoins, absorbing some demand that would otherwise go offshore. The key testable implication is that CBDC launch \emph{increases} total digital settlement volume (onshore plus offshore) rather than merely displacing private stablecoins with a government alternative. \emph{Timing:} Observable within 3 years of any G20 retail CBDC launch. \emph{Evidence against:} total digital settlement volume (CBDC + stablecoins + unsurveilled) growing more slowly after CBDC launch than before, or the displacement ratio falling below 0.3.
