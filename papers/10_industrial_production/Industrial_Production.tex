...
\citet{smirl2026free} parameterizes information frictions by an \emph{information temperature} $T = 1/\kappa$, where $\kappa$ is the Shannon capacity available to the decision-maker. The economic free energy is
\begin{equation}\label{eq:free_energy}
\calF = \Phi_{\mathrm{CES}}(\rho) - T \cdot H
\end{equation}
where $\Phi = -\log F$ is the CES potential and $H$ is Shannon entropy. At $T = 0$: perfect information, deterministic optimization. At $T > 0$: costly information processing, stochastic allocation. The optimal allocation under information constraints takes the logit (Gibbs) form:
...
Under information frictions ($T > 0$), the firm does not observe $\boldsymbol{\theta}$ directly. It acquires information subject to a Shannon capacity constraint $I(\boldsymbol{\theta}; \mathbf{s}) \leq \kappa = 1/T$, where $\mathbf{s}$ is the signal vector. The firm's problem is:
