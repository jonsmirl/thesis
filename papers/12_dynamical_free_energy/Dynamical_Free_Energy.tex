...
\title{CES Potential Dynamics:\\Variance-Response Identities, Early Warning Signals,\\and Multi-Scale Aggregation in Economic Systems}
\author{Jon Smirl}
...
\begin{abstract}
The companion papers establish that the Tsallis CES potential $\calF_q = \Phi_{\mathrm{CES}}(\rho) - T \cdot S_q$ with $q = \rho$ governs equilibrium selection in economies with CES production technology (curvature $K$) and information frictions ($T = 1/\kappa$). This paper develops the \emph{dynamical} consequences of that CES potential. Treating $\calF$ as a landscape on which the economy evolves yields six families of results, each importing established physics machinery into economics with new economic content: (i) \emph{adjustment dynamics} derive relaxation timescales and the multi-level hierarchy from the eigenstructure of $\nabla^2 \calF$; (ii) the \emph{Variance-Response Identity} (FDT; Fluctuation-Dissipation Theorem) predicts that the responsiveness of output to shocks equals productivity variance divided by information friction, operationalizing $T$ as a measurable quantity; (iii) \emph{pre-crisis deceleration} near regime boundaries generates early warning signals---rising autocorrelation and variance---before technology crossings and financial crises; (iv) the \emph{Symmetric Adjustment Theorem} (Onsager) predicts that cross-sector coupling coefficients form a symmetric matrix, a non-obvious testable restriction; (v) \emph{Noise-Driven Transition Rate theory} (Kramers) converts deterministic crossing predictions into probability distributions over transition times; and (vi) \emph{multi-scale aggregation} analysis identifies $\rho$ and $T$ as the only relevant parameters under aggregation from firms to industries to economies, providing the mathematical foundation for macroscopic predictability from microscopic unpredictability. Each result generates specific, quantitative, testable predictions. The framework also establishes a lower bound on policy costs via the Minimum Policy Cost Theorem (Jarzynski): forcing an economic transition costs at least $\Delta \calF$, with excess cost measurable from implementation fluctuations.
\end{abstract}

\textbf{JEL Codes:} C62, D50, E10, E32, O33

\textbf{Keywords:} CES potential, variance-response identity, pre-crisis deceleration, early warning signals, symmetric adjustment, multi-scale aggregation, regime shifts, information friction

%=============================================================================
\section{Introduction}\label{sec:intro}
%=============================================================================

The companion papers establish a two-parameter framework for economic analysis. The CES quadruple role theorem \citep{smirl2026ces} shows that a single curvature parameter $K = (1-\rho)(J-1)/J$ controls superadditivity, correlation robustness, strategic independence, and network scaling. The CES potential framework \citep{smirl2026free} shows that production under information frictions is governed by $\calF_q = \Phi_{\mathrm{CES}}(\rho) - T \cdot S_q$, where $T = 1/\kappa$ is information friction and $S_q$ is Tsallis entropy with $q = \rho$ \citep{smirl2026tsallis}. The effective curvature theorem \citep{smirl2026prod} shows that exploitable curvature is $K_{\mathrm{eff}} = K \cdot (1 - T/T^*)^+$. And the technology cycle paper \citep{smirl2026cycle} derives the Perez phases from bifurcation dynamics on the $(\rho, T)$ regime diagram.

All of these results are \emph{static}: they characterize equilibria, compare equilibria, and identify boundaries between equilibrium regimes. But in physics, the CES potential is not merely a selection criterion---it is the \emph{generating function for the entire dynamical theory}. Adjustment dynamics on the CES potential landscape determine relaxation dynamics. Fluctuations around equilibria satisfy the Variance-Response Identity. Regime shifts exhibit pre-crisis deceleration. Cross-couplings obey the Symmetric Adjustment Theorem. Escape from metastable states follows Kramers transition rate theory. And multi-scale aggregation identifies which parameters survive aggregation.

This paper imports each of these dynamical results into the economic CES potential framework. The economics is not an analogy---the mathematical structure is identical because the CES potential functional has the same form. What changes is the physical content: energy $E$ becomes CES potential $\Phi$, thermodynamic temperature becomes information friction $T = 1/\kappa$, particle configurations become input allocations, and regime shifts become technology crossings.

The paper has a specific empirical payoff: it converts abstract parameters into observables and theoretical predictions into testable restrictions. Information friction $T$, which enters the companion papers as a shadow price of information capacity, becomes measurable from the ratio of output variance to shock response (\Cref{sec:fdt}). The crisis sequence theorem, which predicts the order of failures, becomes an early warning system with specific signals to monitor (\Cref{sec:critical}). The technology crossing, which appears as a deterministic threshold, becomes a stochastic process with a computable probability distribution (\Cref{sec:kramers}). And the claim that $(\rho, T)$ suffices for macroscopic prediction becomes a testable statement about multi-scale aggregation relevance (\Cref{sec:rg}).

\paragraph{Contributions.} The paper makes seven contributions:

\begin{enumerate}[label=(\roman*)]
\item \textbf{Adjustment dynamics} (\Cref{sec:gradient}): The economy's relaxation toward equilibrium follows $\dot{\mathbf{x}} = -\mathbf{L} \nabla \calF$, where $\mathbf{L}$ is a mobility matrix. The eigenvalues of $\mathbf{L} \nabla^2 \calF$ are the relaxation rates, deriving the $N$-level timescale hierarchy from the Hessian of $\calF$.

\item \textbf{Variance-Response Identity} (\Cref{sec:fdt}): At equilibrium, $\chi_{ij} = \sigma_{ij}^2 / T$, where $\chi$ is the response matrix (output response to input shocks) and $\sigma^2$ is the covariance of spontaneous fluctuations. This operationalizes $T$ from observable data.

\item \textbf{Pre-crisis deceleration and early warning signals} (\Cref{sec:critical}): As the system approaches a regime boundary ($T \to T^*$ or $K_{\mathrm{eff}} \to 0$), the dominant relaxation rate vanishes, autocorrelation time diverges, and variance increases. These are detectable precursors to crises.

\item \textbf{Symmetric Adjustment Theorem} (Onsager) (\Cref{sec:onsager}): Near equilibrium, the matrix of cross-sector transport coefficients is symmetric: $L_{ij} = L_{ji}$. A change in complementarity in sector $i$ that induces information flow to sector $j$ has the same coefficient as a change in information capacity in sector $j$ that induces complementarity change in sector $i$.

\item \textbf{Kramers transition rate theory} (\Cref{sec:kramers}): The transition rate from the centralized to the distributed basin is $k = \nu \exp(-\Delta \calF / T)$, converting deterministic crossing predictions into probability distributions over transition times.

\item \textbf{Minimum Policy Cost Theorem} (Jarzynski) (\Cref{sec:jarzynski}): The minimum cost of forcing an economic transition is $\Delta \calF$. Excess cost (deadweight loss) equals the friction work $W_{\mathrm{diss}} = W - \Delta \calF \geq 0$, measurable from the variance of outcomes across policy implementations.

\item \textbf{Multi-scale aggregation analysis} (\Cref{sec:rg}): Under aggregation from firms to industries to economies, $\rho$ and $T$ are the only relevant parameters. All other microscopic details---firm strategies, individual preferences, institutional forms---are irrelevant operators that aggregate to zero. This provides the mathematical foundation for macroscopic predictability.
\end{enumerate}

\paragraph{Roadmap.} \Cref{sec:landscape} establishes the CES potential landscape and its properties. \Cref{sec:gradient} derives adjustment dynamics. \Cref{sec:fdt} proves the economic Variance-Response Identity. \Cref{sec:critical} develops pre-crisis deceleration and early warning signals. \Cref{sec:onsager} establishes the Symmetric Adjustment Theorem. \Cref{sec:kramers} applies Kramers transition rate theory to technology transitions. \Cref{sec:jarzynski} derives the Minimum Policy Cost bound. \Cref{sec:rg} develops the multi-scale aggregation analysis. \Cref{sec:empirical} collects testable predictions. \Cref{sec:literature} discusses the literature. \Cref{sec:conclusion} concludes.

%=============================================================================
\section{The CES Potential Landscape}\label{sec:landscape}
%=============================================================================
...
\end{equation}
with curvature $K_n = (1-\rho_n)(J_n - 1)/J_n$. The CES potential is $\Phi = -\sum_{n=1}^{N} \log F_n$ and the total CES potential is:
\begin{equation}\label{eq:free_energy}
\calF_q(\mathbf{x}; \boldsymbol{\rho}, \mathbf{T}) = \Phi(\mathbf{x}; \boldsymbol{\rho}) - \sum_{n=1}^{N} T_n \cdot S_{q_n}(\mathbf{x}_n)
\end{equation}
where $S_{q_n}$ is the Tsallis entropy of the normalized allocation in sector $n$ with $q_n = \rho_n$.

\subsection{Landscape Geometry}

The CES potential landscape $\calF: \R_{++}^{\sum J_n} \to \R$ has the following properties.

\begin{proposition}[Landscape structure]\label{prop:landscape}
The CES potential $\calF$ defined in \eqref{eq:free_energy} satisfies:
...
\item \textbf{Breakdown threshold:} The breakdown threshold for sector $n$ is:
\begin{equation}\label{eq:Tstar}
T_n^* = \frac{\lambda_{\min}(\nabla^2 \Phi_n)}{\lambda_{\max}(\nabla^2 H_n)}
\end{equation}
where $\lambda_{\min}$ and $\lambda_{\max}$ denote the smallest and largest eigenvalues respectively. At $T_n = T_n^*$, the Hessian acquires a zero eigenvalue---the hallmark of a regime shift.
\end{enumerate}
\end{proposition}
...
%=============================================================================
\section{Adjustment Dynamics}\label{sec:gradient}
%=============================================================================
...
\end{equation}
with equality if and only if $\nabla \calF = 0$ (at equilibrium). The CES potential decreases monotonically until the system reaches a local minimum.
\end{proposition}
...
\end{equation}
where $d\mathbf{W}$ is a vector Wiener process and $\mathbf{T} = \mathrm{diag}(T_1 \mathbf{I}_{J_1}, \ldots, T_N \mathbf{I}_{J_N})$ is the friction matrix. The noise amplitude $\sqrt{2\mathbf{L}\mathbf{T}}$ is not arbitrary---it is fixed by the requirement that the stationary distribution is the logit equilibrium:
\begin{equation}\label{eq:boltzmann}
p_{\mathrm{eq}}(\mathbf{x}) \propto \exp\left(-\calF(\mathbf{x}) / T_{\mathrm{eff}}\right)
\end{equation}
This is the Einstein relation for the economic system: the strength of spontaneous fluctuations is determined by the information friction and the institutional mobility, not by any separate ``shock process.''

\begin{remark}[Economic interpretation of Langevin noise]
The noise in \eqref{eq:langevin} is not exogenous---it arises from the information frictions themselves. Agents with finite information capacity $\kappa = 1/T$ make allocation decisions with inherent randomness (the $q$-logit choice model generalizing \citealt{matejka2015}). When aggregated across many agents, this produces Brownian fluctuations in the aggregate allocation with variance proportional to $T/(2-q)$ where $q = \rho$ \citep{smirl2026tsallis}. Higher information friction means noisier aggregate outcomes; for complementary sectors ($q < 1$), the $1/(2-q) > 1$ factor amplifies fluctuations beyond the Shannon prediction.
\end{remark}

\subsection{The FDT for Economic Systems}

\begin{theorem}[Economic Variance-Response Identity]\label{thm:fdt}
At a stable equilibrium $\mathbf{x}^*$ of the CES potential landscape, the following relation holds:
\begin{enumerate}[label=(\alph*)]
...
\subsection{Operationalizing Information Friction}

\Cref{thm:fdt} has a profound empirical consequence: information friction is measurable from data.

\begin{corollary}[Measuring $T$ from data]\label{cor:measure_T}
For any sector $n$ with observable output variance $\sigma_n^2$ (from firm-level productivity dispersion data) and measurable shock response $\chi_n$ (from natural experiments or policy changes), the information friction is:
...
\subsection{The Approach to the Tipping Point}

As information friction $T_n$ approaches the breakdown threshold $T_n^*$, the effective curvature $K_{\mathrm{eff},n} \to 0$ and the landscape flattens in the direction of the incipient instability. The relaxation rate in that direction vanishes:
...
\end{enumerate}
All divergences are characterized by the divergence rate $\gamma = 1$ (mean-field aggregation-invariant class).
\end{theorem}

\begin{proof}
All four results follow from the spectral decomposition of the linearized dynamics near the tipping point. The autocorrelation function of the slowest mode is $C(t) = \sigma^2 e^{-\lambda_{\min} t}$, giving autocorrelation time $1/\lambda_{\min}$. The equilibrium variance from the FDT is $\sigma^2 = T/(\lambda_{\min}/\ell)$. Recovery time is $t_{\mathrm{rec}} = \ln(\delta/\epsilon)/\lambda_{\min}$. Susceptibility is $\chi = 1/(\lambda_{\min}/\ell)$. All diverge as $(1-T/T^*)^{-1}$ because $\lambda_{\min} \propto (1-T/T^*)$.

The mean-field exponent $\gamma = 1$ follows from the quadratic (Gaussian) nature of the quadratic potential approximation near the tipping point, which is exact for the CES system because the Hessian structure is mean-field-like (all-to-all coupling within each sector).
\end{proof}

\subsection{Early Warning Signals}

\Cref{thm:critical_slowing} translates directly into observable early warning signals (EWS) for economic regime shifts.

\begin{proposition}[Economic early warning signals]\label{prop:ews}
The approach of an economic system to a regime boundary is signaled by:
\begin{enumerate}[label=(\alph*)]
...
\item \textbf{Flickering:} Near the tipping point, stochastic fluctuations push the system between the neighborhood of the vanishing equilibrium and the emerging alternative, producing bimodal distributions in time series that were previously unimodal.
...
%=============================================================================
\section{Symmetric Adjustment Theorem (Onsager)}\label{sec:onsager}
%=============================================================================
...
Define the economic force in sector $n$ as the gradient of the CES potential:
\begin{equation}
X_n = -\nabla_n \calF = -\nabla_n \Phi + T_n \nabla_n H
\end{equation}
...
\begin{theorem}[Symmetric Adjustment Theorem for economic sectors]\label{thm:onsager}
...
The Symmetric Adjustment Theorem is non-obvious economically. Consider two sectors: semiconductor manufacturing (strong complementarity, moderate information friction) and financial services (weak complementarity, low information friction). Reciprocity predicts:
\begin{itemize}
...
Among all steady states maintained by external forces, the one that minimizes entropy production is the one closest to equilibrium (in the metric defined by $\mathbf{L}^{-1}$). A policy that maintains the economy away from its CES potential minimum incurs ongoing entropy production---a flow cost of misallocation.
\end{proposition}
...
%=============================================================================
\section{Noise-Driven Transition Rates (Kramers)}\label{sec:kramers}
%=============================================================================

\subsection{Metastable States and Transition Rates}

When the CES potential landscape has multiple local minima---as occurs near regime boundaries in the $(\rho, T)$ diagram---the system can be trapped in a metastable state. Kramers transition rate theory computes the rate of noise-driven escape.

\begin{theorem}[Economic Kramers transition rate]\label{thm:kramers}
Consider a CES potential landscape with two minima (centralized equilibrium $\mathbf{x}_c$ and distributed equilibrium $\mathbf{x}_d$) separated by a saddle point $\mathbf{x}_s$. The transition rate from the centralized to the distributed basin is:
...
The companion papers give deterministic crossing conditions: distributed production is viable when $T_{\mathrm{dist}} < T^*$. Kramers transition rate theory adds the missing element: even before the deterministic threshold is reached ($\Delta\calF > 0$, the centralized equilibrium basin is still deeper), stochastic fluctuations can trigger early crossing with probability $\sim \exp(-\Delta\calF/T)$ per unit time. Conversely, even after the threshold ($\Delta\calF < 0$, the distributed equilibrium basin is deeper), the actual transition may be delayed by the time required to escape the metastable centralized equilibrium basin.
...
This explains the historical observation that policies attempting to prevent technology transitions (the Red Flag Acts for automobiles, AT\&T monopoly protection for telephony, taxi medallion systems for ride-sharing) typically fail: they raise the barrier $\Delta\calF$ linearly, but the effective information friction $T$ during the frenzy phase rises sufficiently that $\Delta\calF/T$ still decreases over time.

%=============================================================================
\section{The Minimum Policy Cost Theorem (Jarzynski)}\label{sec:jarzynski}
%=============================================================================

\subsection{Non-Equilibrium Work}

When policy forces an economic transition---through subsidies, regulations, or direct intervention---the system is driven out of equilibrium. The Minimum Policy Cost Theorem relates the work performed during such a non-equilibrium process to the equilibrium CES potential difference.
...
\end{equation}
where $\Delta \calF = \calF_B - \calF_A$ is the CES potential difference between the final and initial states, and $\langle \cdot \rangle$ denotes the average over many realizations of the stochastic dynamics under the policy protocol.
\end{theorem}

\begin{proof}
The Minimum Policy Cost Theorem (Jarzynski equality) holds for any system governed by Langevin dynamics with logit equilibrium distribution and time-dependent external forcing \citep{jarzynski1997}. Our economic system satisfies these conditions by construction.
\end{proof}
...
\end{equation}
The average policy cost of any transition is bounded below by the CES potential difference. Equality is achieved only in the quasi-static limit (infinitely slow policy implementation).
\end{corollary}

\begin{definition}[Friction work]\label{def:dissipated_work}
The friction work (deadweight loss of the policy) is:
\begin{equation}\label{eq:dissipated}
W_{\mathrm{diss}} = \langle W \rangle - \Delta \calF \geq 0
\end{equation}
This measures the excess cost of the policy over the theoretical minimum.
\end{definition}

\begin{proposition}[Estimating friction work]\label{prop:dissipated_estimate}
From the Minimum Policy Cost Theorem, the friction work can be estimated from the variance of work across policy implementations:
\begin{equation}\label{eq:diss_variance}
W_{\mathrm{diss}} \approx \frac{\Var(W)}{2T}
\end{equation}
to leading order in the near-equilibrium expansion. This means the deadweight loss of a policy is proportional to the variance of its outcomes divided by information friction.
\end{proposition}
...
The Minimum Policy Cost framework has three practical consequences:
\begin{enumerate}[label=(\roman*)]
...
Consider a policy forcing transition from fossil fuel generation ($\rho_{\text{fossil}}$, $T_{\text{fossil}}$) to renewable generation ($\rho_{\text{renew}}$, $T_{\text{renew}}$). The CES potential difference $\Delta\calF$ depends on the curvature gap and information friction gap between the two technologies. The Minimum Policy Cost bound says the minimum subsidy cost is $\Delta\calF$---but different policy designs (feed-in tariffs, renewable portfolio standards, carbon taxes) achieve different realizations $W$, with $\langle W \rangle - \Delta\calF$ measuring the policy's inefficiency. Cross-country variation in renewable subsidy costs, controlling for $\Delta\calF$, directly estimates $W_{\mathrm{diss}}$.
\end{example}

%=============================================================================
\section{Multi-Scale Aggregation Analysis}\label{sec:rg}
%=============================================================================

\subsection{The Aggregation Problem}

The CES potential $\calF$ is defined at the level of individual firms combining individual inputs. But macroeconomic predictions---technology crossings, crisis timing, cycle duration---operate at the level of industries or economies. Multi-scale aggregation provides the mathematical machinery for connecting these scales: which microscopic parameters survive aggregation, and which are ``washed out''?

\begin{definition}[Aggregation operator]\label{def:coarse_grain}
Define an aggregation operator $\calR_b$ that aggregates $b^d$ firms within each sector into a single ``block firm,'' where $b > 1$ is the blocking factor and $d$ is the number of spatial or organizational dimensions:
\begin{equation}\label{eq:coarse_grain}
\calR_b: \calF(\{x_{nj}\}; \rho_n, T_n) \mapsto \calF'(\{X_{nJ}\}; \rho_n', T_n')
\end{equation}
where $X_{nJ}$ are the aggregated (block) allocations and $(\rho_n', T_n')$ are the aggregated parameters.
\end{definition}

\subsection{Aggregation Flow Equations}

\begin{theorem}[Aggregation of $(\rho, T)$]\label{thm:rg}
Under the aggregation operator $\calR_b$, the parameters flow according to:
\begin{enumerate}[label=(\alph*)]
\item \textbf{Complementarity parameter:}
\begin{equation}\label{eq:rg_rho}
\rho' = \rho + \delta\rho(\rho, T, b)
\end{equation}
where $\delta\rho$ is determined by the change in effective curvature under aggregation. For the CES aggregate, the key property is \emph{self-similarity}: a CES aggregate of CES aggregates is again CES (up to corrections that vanish at the fixed point). The aggregated curvature is:
\begin{equation}\label{eq:rg_K}
K' = K \cdot \left(1 - \frac{T}{T^*}\right) + O(K^2)
\end{equation}
The effective curvature decreases under aggregation when $T > 0$: information frictions at the micro level erode complementarity at the macro level.

\item \textbf{Information friction:}
\begin{equation}\label{eq:rg_T}
T' = T \cdot b^{y_T}
\end{equation}
where $y_T > 0$ is the friction scaling exponent. Information frictions are a \emph{relevant} perturbation: they grow under aggregation. This means that even small micro-level frictions become important at the macro level.
...
\end{equation}
They shrink under aggregation and vanish in the macroscopic limit.
\end{enumerate}
\end{theorem}
...
\subsection{Fixed Points and Aggregation-Invariant Classes}
...
\item \textbf{Tipping point fixed point} ($K^* > 0$, $T^*/T = 1$): The regime boundary between the productive and the information-dominated regime. Systems near this fixed point exhibit the tipping phenomena of \Cref{sec:critical}: diverging correlation length, power-law distributions, and universal scaling.
...
Technologies with the same $(\rho, T/T^*)$ but different microscopic details (firm sizes, organizational structures, cultural factors) belong to the same \emph{aggregation-invariant class} and exhibit identical macroscopic dynamics---the same cycle shape, crisis sequence, and duration scaling.  The deeper reason CES survives aggregation is that it is the \emph{unique} homogeneous, scale-consistent aggregator (the Kolmogorov--Nagumo--Acz\'{e}l theorem); non-CES production functions are irrelevant perturbations guaranteed to vanish at the macro level \citep{smirl2026emergent}.
\end{corollary}
...
\end{equation}
where $\calF^*$ is the fixed-point CES potential and the corrections decay as power laws with exponents $|y_s| > 0$.
...
\begin{proof}
This follows from the standard aggregation argument: repeated application of $\calR_b$ drives all irrelevant operators to zero, leaving only the relevant parameters $(\rho, T, \ell)$ in the effective CES potential. The convergence rate is $b^{-|y_s|}$ for the least irrelevant operator.
\end{proof}

\begin{remark}[The psychohistory connection]
\Cref{thm:predictability} formalizes the intuition that motivates macroeconomic prediction: large-scale outcomes are insensitive to individual-level details. Multi-scale aggregation provides the mechanism: aggregation eliminates microscopic information while preserving the relevant parameters. What the $(\rho, T)$ framework adds is the identification of \emph{which} parameters are relevant. In the Landau-Ginzburg classification, $\rho$ determines the aggregation-invariant class (what kind of transition occurs) and $T$ determines the distance from the tipping point (how close the transition is). Everything else is irrelevant in the precise technical sense.
\end{remark}
...
\begin{enumerate}[label=(\alph*)]
\item \textbf{Information friction from data:} For each industry $n$, the ratio $T_n = \sigma_n^2 / \chi_n$ (productivity variance over shock responsiveness) should be stable across different shock types and time periods, yielding a consistent sector-specific $T_n$.

\item \textbf{T declines with digitization:} Industries that adopt information technology should show declining $T_n$ over time, as $T = 1/\kappa$ and digitization increases information capacity $\kappa$.
...
\begin{remark}[Preliminary evidence: cross-sectional FDT]
Using monthly FRED Industrial Production data for 17 manufacturing subsectors (1972--2026), we estimate sector-specific $\sigma_n^2$ (variance of log-growth rates) and $\chi_n$ (cumulative impulse response to aggregate output shock from bivariate VARs with BIC-selected lag order).  The cross-sectional OLS regression $\sigma_n^2 = \alpha + T \cdot \chi_n + \varepsilon_n$ yields $\hat{T} = 1.04 \times 10^{-4}$ with correct (positive) sign but low explanatory power ($R^2 = 0.009$, $p = 0.72$, $N = 17$).  The directional result is consistent with the FDT, but power is limited by sample size and the use of aggregate (rather than firm-level) data.  A sharper test requires firm-level production function estimates, as specified in \cref{prop:fdt_predictions}(c)--(d).
\end{remark}

\begin{remark}[Stronger evidence: self-consistent information friction from EEMD]\label{rem:fdt_eemd}
A more powerful validation of the FDT uses the EEMD decomposition developed in the companion paper \citep{smirl2026emd}.  EEMD partitions INDPRO growth into fast modes (period $< 4$ years, 93.6\% of energy) and slow structural modes (period $> 8$ years, 3.3\% of energy).  The fast-mode Hilbert energy $T_{\mathrm{fast}}(t) = \sum_{k \in \mathrm{fast}} a_k(t)^2$ is the self-consistent information friction---the exact analogue of kinetic temperature in molecular dynamics.  The FDT susceptibility $\chi(t) = E_{\mathrm{slow}}(t)/T_{\mathrm{fast}}(t)$ detects five technology wave peaks (1938, 1948, 1977, 1994, 2006) that all correspond to known technology eras.  Critically, the self-consistent $T$ automatically discriminates technology waves from financial crises: during crises (1929, 2008), $T_{\mathrm{fast}}$ spikes in the denominator, suppressing $\chi$; during technology waves, only $E_{\mathrm{slow}}$ rises.  This is exactly the FDT prediction---crisis and structural dynamics live in different spectral bands---validated over 106 years of data without any external proxy.
\end{remark}

\begin{remark}[Empirical evidence: $T$ as recession leading indicator]\label{rem:T_probit}
The operational information friction $T(t) = \sigma^2(t)/\chi(t)$, computed from rolling 60-month sector-level variance and responsiveness across 7 manufacturing subsectors (1972--2026), has significant predictive content for recessions.  A probit model for 12-month-ahead NBER recession indicators yields $\hat{\beta}(T) = 221$ ($p = 0.0003$, pseudo-$R^2 = 0.035$).  In a joint specification with the term spread (10Y--2Y Treasury), pseudo-$R^2$ rises to 0.186, compared to 0.107 for the term spread alone and 0.0001 for the VIX alone.  The information friction thus adds substantial forecasting power beyond standard financial indicators.  This is consistent with the FDT: $T$ captures the misallocation friction that financial spreads and volatility indices measure only indirectly.
\end{remark}
...
The EEMD-based CES potential wave detector \citep{smirl2026emd} provides empirical support for the pre-crisis deceleration prediction.  The autocorrelation decay time $\tau_{\mathrm{ACF}}(t)$ of the slow EEMD modes (period $> 8$ years) peaks simultaneously with the FDT susceptibility at all five detected technology waves (1938, 1948, 1977, 1994, 2006).  The combined indicator $\Psi = \sqrt{\hat{\chi} \cdot \hat{\tau}}$ requires both signatures to be elevated, confirming that technology transitions exhibit the joint susceptibility/slowing signature predicted by \Cref{thm:critical_slowing}.  Multi-sector eigenvector analysis further confirms the symmetry-breaking component: the principal eigenvector of slow-mode cross-sector covariance rotates between technology eras, with Computer/Electronics dominant at 1989 and 2020 (IT and AI waves).
\end{remark}

\begin{remark}[Empirical evidence: pre-recession deceleration in manufacturing]\label{rem:ews_manufacturing}
The pre-crisis deceleration prediction extends from technology waves to business cycle recessions.  Using rolling 120-month AR(1) estimates for 7 manufacturing IP subsectors spanning the $\rho$ range $[-0.2, 0.7]$, we test whether AR(1) coefficients rise in the 36 months preceding NBER peaks.  Across 3 recessions (1990, 2001, 2007) and 7 sectors (21 sector-recession pairs), 14/21 (67\%) show rising AR(1) in the pre-crisis window (Kendall $\tau(\text{AR}(1), \text{time}) > 0$).  The signal is strongest for the 2007--09 recession (6/7 sectors, mean $\tau = +0.31$) and the 2001 recession (5/7 sectors, mean $\tau = +0.13$).  The 1990--91 recession is an exception (only 3/7 sectors), possibly because the mild, inventory-driven nature of that recession did not produce the landscape flattening that generates pre-crisis deceleration.  These results provide sector-level confirmation of \Cref{thm:critical_slowing}: the approaching crisis manifests as longer relaxation times in the production data, observable months before the NBER peak.
\end{remark}

\subsection{From the Symmetric Adjustment Theorem}

\begin{proposition}[Symmetric Adjustment predictions]\label{prop:onsager_predictions}
\begin{enumerate}[label=(\alph*)]
...
We estimate an 8-variable VAR (BIC lag order 1) on the durable manufacturing subsectors (Primary Metals through Miscellaneous Manufacturing) using monthly FRED Industrial Production data (1972--2026, $n = 648$).  The cumulative impulse response matrix $L$ at horizon $h = 12$ yields 28 off-diagonal pairs $(L_{ij}, L_{ji})$ with $i < j$.  Pearson correlation $r(L_{ij}, L_{ji}) = 0.067$ ($p = 0.73$), with relative asymmetry $\|L - L^\top\|/\|L\| = 1.43$.  The result does not support approximate symmetry.  This may reflect violation of the near-equilibrium condition: durable manufacturing experienced large structural shifts over this period, and the Cholesky identification imposes an ordering that can systematically break the symmetry that the theory predicts for identified (structural) shocks.  A test using external instruments for sector-specific shocks would be more appropriate.
\end{remark}

\subsection{From Kramers Transition Rate Theory}

\begin{proposition}[Kramers transition rate predictions]\label{prop:kramers_predictions}
\begin{enumerate}[label=(\alph*)]
...
\subsection{From Multi-Scale Aggregation}

\begin{proposition}[Aggregation predictions]\label{prop:rg_predictions}
\begin{enumerate}[label=(\alph*)]
\item \textbf{Aggregation invariance:} Countries with different institutional structures but similar $(\rho, T)$ for a given technology should exhibit the same technology cycle dynamics (same regime ordering, same duration scaling, same crisis sequence).
...
\paragraph{Variance-response relations in economics.} \citet{bouchaud2018} discusses fluctuation-response relations in financial markets, finding approximate FDT-like behavior in price dynamics. Our contribution is to derive the Variance-Response Identity from the specific CES potential structure of CES production under rational inattention, giving the theorem economic content ($T$ is \emph{information friction}) and identifying what FDT violations mean economically (departure from near-equilibrium or breakdown of rational inattention).

\paragraph{Early warning signals.} The ecological EWS literature \citep{scheffer2009, dakos2012} establishes pre-crisis deceleration as a generic precursor to regime shifts. Applications to financial markets include \citet{battiston2016} and others. Our contribution is the \emph{ordered} EWS from the non-uniform degradation theorem: the framework predicts not just that signals exist but which variable signals first and by how much.

\paragraph{Symmetric adjustment in economics.} To our knowledge, this is the first explicit derivation of the Symmetric Adjustment Theorem (Onsager reciprocal relations) for cross-sector economic coupling. The closest precedent is \citet{samuelson1947}, who noted that the symmetry of the Slutsky matrix is analogous to reciprocal relations, but did not develop the dynamical (transport coefficient) version.

\paragraph{Kramers transition rate theory and economic transitions.} The application of Kramers transition rate theory to economic models is novel. The closest work is the literature on stochastic exit from currency pegs \citep{krugman1991}, which uses a related barrier-crossing framework but without the CES potential structure that connects barrier height to economic fundamentals.

\paragraph{Multi-scale aggregation in economics.} \citet{brock1999} and \citet{durlauf1999} discuss aggregation ideas in the context of social interactions. Our application is more specific: the CES aggregation structure provides a natural aggregation, and the relevance/irrelevance classification follows from the scaling properties of $K$ and $T$ under aggregation.

\paragraph{Rational inattention.} The Sims program \citep{sims2003, matejka2015, mackowiak2009} provides the micro-foundation for our information friction. Our contribution is the dynamical extension: rational inattention generates not just static allocation rules but a Langevin dynamics whose fluctuation and friction properties are linked by $T = 1/\kappa$.
...
\item \citet{smirl2026free}: Static---proves the CES potential structure $\calF = \Phi - TH$.
...
\item \textbf{This paper}: Stochastic dynamics---derives variance-response identities, early warning signals, symmetric adjustment relations, transition rates, and multi-scale aggregation from the same $\calF$.
\end{itemize}
...
This paper has shown that treating the economic CES potential $\calF = \Phi_{\mathrm{CES}}(\rho) - T \cdot H$ as a dynamical landscape---rather than merely an equilibrium selection criterion---generates a rich set of results, each with specific testable predictions.

The results fall into three categories by their relationship to observables.

\emph{Results that operationalize abstract quantities.} The Variance-Response Identity converts information friction $T$ from a theoretical construct into a measurable quantity: the ratio of productivity variance to shock responsiveness. This opens the possibility of tracking $T$ across industries and over time, providing an empirical foundation for the entire framework.

\emph{Results that generate early warning signals.} Pre-crisis deceleration, combined with the non-uniform degradation hierarchy, produces an \emph{ordered} early warning system for economic regime shifts. The sequence---rising return correlations, then rising productivity dispersion, then governance instability---is specific enough to be falsified against historical data and applied prospectively to the AI transition.

\emph{Results that constrain policy.} The Minimum Policy Cost Theorem (Jarzynski) establishes lower bounds on policy costs: no intervention can force a technology transition cheaper than the CES potential difference $\Delta\calF$, and the excess cost of any specific policy is measurable from the variance of its outcomes. The Kramers transition rate shows that small reductions in transition barriers produce exponentially large changes in transition probability, while barrier-raising policies face exponentially growing fluctuations during the frenzy phase.

The multi-scale aggregation analysis ties everything together: $\rho$ and $T$ are the only parameters that survive aggregation from the firm level to the macroscopic level. Individual behavior, institutional details, and cultural factors are irrelevant operators in the precise technical sense---they wash out under aggregation. This is why macroscopic economic prediction is possible despite microscopic unpredictability, and it provides the mathematical foundation for the empirical success of the companion papers' two-parameter predictions.

The framework is falsifiable at multiple levels. The Variance-Response Identity can be tested with existing firm-level data and identified shocks. The EWS predictions can be tested against historical crises. The Symmetric Adjustment Theorem can be tested with input-output data. The aggregation predictions can be tested by comparing technology adoption patterns across countries with different institutions but similar technologies. If these tests fail, the failure will be informative: FDT violation indicates departure from near-equilibrium; Symmetric Adjustment violation indicates breakdown of detailed balance (path-dependent dynamics); aggregation prediction failure indicates that some ``irrelevant'' parameter is in fact relevant, requiring extension of the framework.

The deepest implication is epistemological. The $(\rho, T)$ framework does not claim that economies are physical systems. It claims that the mathematical structure governing production under information frictions has the same form as the CES potential of a statistical mechanical system, and therefore inherits the dynamical theorems that physicists have spent 150 years developing. The economic content---what $\rho$ means, what $T$ means, what constitutes a ``regime shift''---is entirely economic. The mathematics is borrowed; the economics is original.
...
\bibitem[Smirl(2026i)]{smirl2026tsallis}
Smirl, Jon. 2026i. ``The Tsallis Free Energy: Non-Extensive Information Costs for Complementary Production.'' Working Paper.
...
\bibitem[Smirl(2026e)]{smirl2026emd}
Smirl, Jon. 2026e. ``Empirical Mode Decomposition of the Economic Timescale Hierarchy.'' Working Paper.
