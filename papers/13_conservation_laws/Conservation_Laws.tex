...
\end{equation}
where $H_n = -\sum_j p_{nj} \log p_{nj}$ is the Shannon entropy of the normalized allocation $p_{nj} = x_{nj}/\sum_k x_{nk}$.
...
Part (b): The CES aggregate with equal weights is a symmetric function. Shannon entropy is a symmetric function of its arguments.
...
\end{enumerate}
Systematic violation of the $(J_n - 1, 1)$ structure indicates that the equal-weight CES assumption is wrong for that sector---the inputs are not symmetric, and the general-weight CES from \citet{smirl2026ces} (Section 8) should be used instead.
\end{remark}
...
\begin{enumerate}[label=(\alph*)]
\item \textbf{Cross-sectional $T$ estimation:} For each sector, compute $\mathbf{x}^* \cdot \nabla H$ from input allocation data. The ratio $T_n^{\mathrm{Euler}} = -1/(\mathbf{x}_n^* \cdot \nabla_n H_n)$ should be positive and economically reasonable (finite Shannon capacity).

\item \textbf{Consistency with FDT:} The Euler estimate $T_n^{\mathrm{Euler}}$ and the FDT estimate $T_n^{\mathrm{FDT}} = \sigma_n^2/\chi_n$ should agree. Systematic disagreement identifies sectors that are out of equilibrium or where the CES assumption fails.
