...
\begin{abstract}
The CES complementarity parameter $\rho$ has been treated as exogenous throughout the economic CES potential framework: a structural constant that determines superadditivity, correlation robustness, strategic independence, breakdown threshold, crisis sequence, and cycle frequency.  This paper endogenizes $\rho$.  Four channels drive $\rho$ evolution: firm-level optimization (firms choose production architecture to minimize CES potential), technological standardization (Wright's Law learning curves increase modularity), evolutionary selection (the Price equation selects for profitable complementarity levels), and multi-scale aggregation (the RG flow pulls the effective $\rho$ toward Cobb-Douglas under aggregation).  The central result is that these channels operate on different timescales and in opposite directions, creating an \emph{endogenous cycle in $(\rho, T)$ space} that reproduces the Perez technology cycle.  Short-run optimization pushes $\rho$ down during expansions (complementary production is more productive when information is cheap), while long-run standardization pushes $\rho$ up (cumulative investment creates modular, substitutable architectures).  The tension between them drives the system into orbit around the critical curve $T^*(\rho)$, producing \emph{endogenous tipping}: the economy spends most of its time near the regime boundary, generating the power-law statistics of economic fluctuations documented by \citet{gabaix2009}.  The endogenous $\rho$ dynamics close the framework: $\rho$ governs economic structure through the CES production function, economic structure determines investment, investment drives technological change through Wright's Law, and technological change determines $\rho$.  The resulting self-referential system has no free parameters---everything is determined by initial conditions and the speed of learning.
\end{abstract}

\textbf{JEL Codes:} O33, E32, D24, L23, C62

\textbf{Keywords:} endogenous complementarity, CES production, endogenous tipping, technology cycles, modularity, standardization, Price equation, Perez waves

%=============================================================================
\section{Introduction}\label{sec:intro}
%=============================================================================

The economic CES potential framework rests on a single structural parameter: the CES complementarity $\rho$.  Through the curvature $K = (1 - \rho)(J-1)/J$, this parameter simultaneously controls the superadditivity of diverse inputs, the robustness of diversification against correlated shocks, and the strategic stability of balanced allocation \citep{smirl2026ces}.  The breakdown threshold $T^*(\rho)$ at which efficient allocation breaks down, the ordering of sectoral crises, the frequency of business cycles, and the topology of technology waves all flow from $\rho$ \citep{smirl2026dynamical,smirl2026conservation,smirl2026business}.
...
This paper identifies four channels through which $\rho$ evolves, derives the coupled $(\rho, T)$ dynamics, and shows that the resulting system exhibits endogenous tipping: the economy is attracted to its own critical curve $T^*(\rho)$, generating power-law fluctuations and reproducing the Perez technology cycle as an endogenous cycle in parameter space.
...
The endogenous tipping result connects to \citet{bak1987}, who showed that driven systems with friction can self-organize to tipping points.  \citet{scheinkman2014} and \citet{gabaix2009} documented power-law regularities in economic data consistent with criticality.  The present paper provides a \emph{mechanism}---endogenous $\rho$ dynamics orbiting the critical curve $T^*(\rho)$---rather than simply observing the statistical signature.

\paragraph{Outline.}
\Cref{sec:channels} identifies the three channels of $\rho$ evolution.  \Cref{sec:rho-equation} derives the equation of motion for $\rho$.  \Cref{sec:coupled} analyzes the coupled $(\rho, T)$ dynamics.  \Cref{sec:limit-cycle} proves the existence of an endogenous cycle and identifies it with the Perez technology cycle.  \Cref{sec:soc} derives endogenous tipping.  \Cref{sec:price} develops the evolutionary perspective through the Price equation.  \Cref{sec:diversity} shows that $\rho$-diversity is itself a source of economic value.  \Cref{sec:closure} explains how endogenous $\rho$ closes the framework.  \Cref{sec:predictions} presents testable predictions.  \Cref{sec:conclusion} concludes.
...
\end{equation}
where $\calF(\rho, T) = \Phi_{\mathrm{CES}}(\rho) - T \cdot H(\rho)$ is the CES potential and $c_A(\rho)$ is the cost of architectural modularity (increasing in $\rho$, since modularity requires interface design, standardization effort, and coordination overhead).
\end{definition}
...
\end{equation}
The denominator is negative (concavity of $\calF$ minus convexity of $c_A$).  For the numerator: at the logit equilibrium, higher $T$ increases the entropy bonus of substitutable architectures (high $\rho$) relative to complementary ones (low $\rho$), making $\partial^2 \calF/\partial \rho \partial T > 0$.  Therefore $d\rho^*/dT > 0$.
\end{proof}
...
The direction of selection depends on the current information friction $T$:
...
%=============================================================================
\subsection{Channel 4: Multi-scale aggregation}\label{sec:rg-channel}

A fourth channel operates not within a scale but \emph{across} scales.  The multi-scale aggregation group (RG) shows that CES is the unique fixed point of aggregation \citep{smirl2026emergent}: when fine-grained inputs are aggregated into blocks, only the power-mean structure survives.  Non-CES deviations are irrelevant operators that vanish under aggregation.
...
The information friction $T$ evolves endogenously through the mechanisms identified in the companion papers \citep{smirl2026dynamical,smirl2026business}:
...
\item If the Minsky drift is strong enough to make the fixed point unstable, the spiral is \emph{outward}, and the system approaches an \emph{endogenous cycle}.
\end{itemize}

%=============================================================================
\section{The Endogenous Cycle: Technology Waves in $(\rho, T)$ Space}\label{sec:limit-cycle}
%=============================================================================

\subsection{Existence of the endogenous cycle}\label{sec:existence}

\begin{theorem}[Endogenous cycle in $(\rho, T)$ space]\label{thm:limit-cycle}
...
\end{enumerate}
Then the system possesses a stable endogenous cycle $\Gamma$ in $(\rho, T)$ space.  The cycle orbits the fixed point and crosses the critical curve $T^*(\rho)$ at two points: entering crisis (from below) and exiting crisis (from above).
\end{theorem}
...
\emph{Step 3: Poincar\'{e}--Bendixson.}  A two-dimensional autonomous system with a positively invariant trapping region containing a single unstable fixed point must possess a stable endogenous cycle, by the Poincar\'{e}--Bendixson theorem.
\end{proof}

\subsection{The four phases}\label{sec:four-phases}

The endogenous cycle $\Gamma$ traverses four quadrants of $(\rho, T)$ space relative to the fixed point $(\bar{\rho}_0, T_0)$, reproducing the four phases of the Perez technology cycle:

\begin{theorem}[Perez phases as quadrants]\label{thm:perez}
The endogenous cycle $\Gamma$ from \cref{thm:limit-cycle} has four phases corresponding to the sign of $(d\rho/dt, dT/dt)$:
...
\item \textbf{Turning point/Crisis} ($d\rho/dt > 0$, $dT/dt > 0$): $T$ crosses $T^*(\bar{\rho})$.  Crisis dynamics activate.  Firms rapidly abandon complementary structures ($\rho$ jumps up).  $T$ spikes as information quality collapses.  This is the fast phase of the slow-fast oscillation.
...
The total period of the endogenous cycle is approximately:
...
The asymmetric timescales (slow sub-critical attraction, fast super-critical repulsion) produce the characteristic pattern of endogenous tipping: long quiet periods near the critical curve punctuated by brief avalanches (crises).
\end{proof}

\subsection{Power-law statistics}\label{sec:power-laws}

\begin{corollary}[Power-law fluctuations]\label{cor:power-laws}
A system exhibiting endogenous tipping near the curve $T^*(\rho)$ produces fluctuations with power-law statistics.  Specifically:
\begin{enumerate}[nosep]
...
With endogenous $\rho$, the economic CES potential framework becomes a closed self-referential system:
...
\begin{enumerate}[nosep]
\item $\rho \to \calF$: the CES potential \citep{smirl2026free};
\item $\calF \to x^*$: logit equilibrium allocation \citep{smirl2026free};
...
The CES potential $\calF = \Phi(\rho) - T \cdot H$ becomes a function on a trajectory in $(\rho, T)$ space rather than a function at a fixed $\rho$.  The equilibrium is a moving target: as $\rho$ evolves, the logit distribution shifts, and the economy must continuously re-equilibrate.  The adjustment time for re-equilibration after a $\rho$ change scales as $1/r_n$ (the within-sector friction), imposing a constraint: $\rho$ cannot change faster than the economy can re-equilibrate, or the system falls out of local equilibrium---precisely the condition for crisis.

\paragraph{Paper 12 (Dynamical Free Energy) \citep{smirl2026dynamical}.}
The multi-scale aggregation flow now has a fully determined attractor.  Paper 12 identified $\rho$ and $T$ as the only relevant parameters under aggregation, but did not specify their dynamics.  With the coupled $(\rho, T)$ system of \eqref{eq:coupled}, the RG flow has an endogenous-cycle attractor---the Perez wave---and the endogenous tipping curve $T^*(\rho)$ as its organizing center.
...
Using monthly FRED Industrial Production data for 8 durable manufacturing subsectors (1972--2026), we test the countercyclicality of $\rho$ via two methods.  \emph{Method A} (correlation proxy): rolling 60-month average pairwise correlation among the 8 sectors serves as a proxy for $\rho$ (higher correlation $\approx$ higher complementarity $\approx$ lower $\rho$ in CES terms).  Kendall $\tau$ between average correlation and rolling INDPRO growth yields $\tau = -0.168$ ($p = 0.014$, $n = 99$ windows), confirming that inter-sector correlation rises in downturns.  \emph{Method B} (direct CES estimation): rolling 120-month window nonlinear least squares estimation of the CES production function applied to durable manufacturing yields 45 $\hat{\rho}$ estimates spanning $[-0.54, 0.99]$.  Kendall $\tau(\hat{\rho}, \text{INDPRO growth}) = -0.313$ ($p = 0.002$), confirming that $\rho$ rises (complementarity increases) during contractions.  Both methods support the prediction $\beta < 0$: complementarity is countercyclical.
\end{remark}
...
\subsection{Prediction 4: Endogenous tipping produces power laws}\label{sec:pred-power}
...
The calibration table from \citet{smirl2026cycle} provides 4 historical $\rho$ values: railroads ($\rho = -2.0$), electrification ($\rho = -1.0$), telephony ($\rho = -0.5$), and internet ($\rho = 0.0$).  All are $\leq 0$, well within the complementary regime and consistent with this prediction.  Rank correlations are in the expected direction: Kendall $\tau(\alpha, \rho) = 0.67$ (more modular technologies learn faster) and $\tau(\text{start year}, \rho) = 0.67$ (later technologies arrive with higher $\rho$), though $p = 0.33$ reflects the small sample ($n = 4$).  These values are author-calibrated rather than independently estimated, so the test is directional rather than confirmatory.  See \texttt{test\_tech\_cycle\_predictions.py}, Test~\#25.
\end{remark}
...
This paper has endogenized the CES complementarity parameter $\rho$, closing the self-referential loop at the heart of the economic CES potential framework.  Three channels drive $\rho$ evolution: firm-level optimization (fast, procyclical), evolutionary selection (medium, $T$-dependent), and technological standardization (slow, secular).  The interaction of these channels, operating on different timescales and with opposite signs at different points in the cycle, produces the main results.

\emph{First}, the coupled $(\rho, T)$ dynamics possess a stable endogenous cycle (\cref{thm:limit-cycle}), which is the Perez technology cycle viewed in parameter space.  The four Perez phases---irruption, frenzy, turning point, deployment---correspond to the four quadrants of the cycle in $(\rho, T)$ space (\cref{thm:perez}).

\emph{Second}, the system exhibits endogenous tipping (\cref{thm:soc}): the endogenous $\rho$ dynamics attract the economy toward its own critical curve $T^*(\rho)$, producing long quiet periods near the regime boundary punctuated by brief crises.  The resulting fluctuations have power-law statistics (\cref{cor:power-laws}), providing a mechanism for the scale-free regularities documented in economic data.
