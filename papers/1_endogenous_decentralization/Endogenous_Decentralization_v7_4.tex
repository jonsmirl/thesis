...
\textbf{Structural determination of $S_T$.} The persistence of training revenue is not a free parameter---it is determined by the production structure of training versus inference. In the companion CES potential framework (Smirl 2026a,b), a production process with CES curvature parameter $K = (1-\rho)(J-1)/J$ operated at information friction $T$ achieves effective curvature $K_{\text{eff}} = K \cdot (1 - T/T^*(\rho))^+$. Training has very low $\rho$ (near-Leontief complementarity: all gradient updates must synchronize) and an enormous temperature gap between centralized ($T_{\text{cent}} \approx 0$, nanosecond NVLink) and distributed ($T_{\text{dist}} \gg T^*$, millisecond WiFi) operation. The quasi-rent from centralization is therefore:
...
\end{equation}
which is large and positive as long as the synchronization gap persists. The three scenarios in Section~3.7 correspond to: \emph{high} $S_T$ when $\rho_{\text{train}} \ll 0$ and $T_{\text{dist}} \gg T_{\text{cent}}$ (current conditions); \emph{moderate} when asynchronous training algorithms raise the effective $\rho$ (reducing $K$); \emph{zero} only if $T_{\text{dist}} \to T_{\text{cent}}$ (synchronization gap closes). The current 5--6 order-of-magnitude latency gap places us firmly in the high-$S_T$ regime, with gradual migration toward moderate as asynchronous methods mature.
...
At $Q = \bar{Q}$, a new equilibrium basin---the distributed inference equilibrium---becomes accessible. Reversing the crossing would require cumulative production to decrease---which contradicts monotonicity. Once $Q$ crosses $\bar{Q}$, the inference transition is structurally irreversible.
\end{remark}
...
The adoption rate has a structural derivation from information-constrained choice. A potential adopter must evaluate two alternatives---centralized (cost $c^*$, latency $\ell_{\text{cent}}$) and distributed (cost $c(Q)$, latency $\ell_{\text{dist}}$)---under limited information about quality, reliability, and deployment cost. Under rational inattention (Sims 2003; Mat\v{e}jka and McKay 2015), the optimal adoption probability takes the logit form:
\begin{equation}\label{eq:logit_adoption}
P(\text{distributed}) = \frac{\exp(\Delta u / T_u)}{1 + \exp(\Delta u / T_u)}
\end{equation}
where $\Delta u = u_{\text{dist}} - u_{\text{cent}}$ is the utility differential and $T_u = 1/\kappa_u$ is the user's information friction (how well they can evaluate the alternatives). The utility differential decomposes into cost and quality channels:
\begin{equation}\label{eq:delta_u}
\Delta u = \underbrace{\beta_0 (c^* - c(Q))}_{\text{cost advantage}} + \underbrace{\lambda(\ell_{\text{cent}} - \ell_{\text{dist}})}_{\text{latency advantage}}
\end{equation}
For small adoption shares $s \ll 1$, the logit probability approximates the linear form $P \approx \Delta u / T_u$ when $\Delta u \ll T_u$, recovering the specification:
\begin{equation}\label{eq:beta}
\beta(c, \lambda) = \beta_0 \cdot \left(c^* - c(Q)\right)^+ + \lambda
\end{equation}
with $\beta_0 = \tilde{\beta}_0/T_u$ and $\lambda = \tilde{\lambda}/T_u$, where tildes denote the primitive parameters.

The rational inattention derivation provides three structural insights absent from the reduced form. First, $\beta_0$ and $\lambda$ are \emph{jointly determined} by the user's information capacity $\kappa_u$---they are not independent free parameters. Second, the coordination friction $\kappa$ in the adoption dynamics \eqref{eq:sdynamics} maps directly to $T_u$: confused users ($T_u$ high) adopt slowly because they cannot distinguish quality. Third, the ReLU $(\cdot)^+$ emerges from the regime shift in the companion CES potential framework (Smirl 2026a): below cost parity, adoption is thermally activated (exponentially suppressed in $|\Delta u|/T_u$); above it, adoption is first-order in the cost advantage.
...
\emph{Organizational viability.} The cost-parity and $R_0$ conditions address whether distributed production is \emph{affordable} and whether adoption is \emph{self-sustaining}. A third condition, from the companion effective curvature framework (Smirl 2026b), asks whether distributed production is \emph{organizationally viable}: the mesh of distributed devices must coordinate well enough to match centralized output quality. Formally, distributed production of task type $\tau$ is viable when:
\begin{equation}\label{eq:org_viability}
K_{\text{eff}}(\rho_\tau, T_{\text{mesh}}) \geq K_{\text{threshold}}
\end{equation}
where $K_{\text{eff}} = K \cdot (1 - T_{\text{mesh}}/T^*(\rho_\tau))^+$ is the effective curvature under mesh coordination temperature $T_{\text{mesh}}$. For inference ($\rho \approx 1$, $K \approx 0$), condition \eqref{eq:org_viability} is automatically satisfied: there is no complementarity to exploit, so organizational viability imposes no additional constraint. This validates the paper's approach of using cost parity as the crossing condition for inference. For more complex distributed tasks---federated fine-tuning, multi-agent collaboration, distributed evaluation---$\rho < 1$ and condition \eqref{eq:org_viability} becomes binding. The generalized crossing threshold is:
...
\subsection{Formal Derivation from the Effective Curvature Framework}

The training-inference distinction---asserted above from architectural characteristics---can be derived as a theorem from the CES effective curvature framework developed in the companion papers (Smirl 2026a,b; Smirl 2026c). The key insight is that training and inference occupy opposite corners of the $(\rho, T)$ production regime diagram.

\begin{proposition}[Training-Inference Bifurcation]\label{prop:bifurcation}
Let $K_{\text{eff}}(\rho, T) = K \cdot (1 - T/T^*(\rho))^+$ be the effective curvature of a CES production technology with structural curvature $K = (1-\rho)(J-1)/J$ operated at information friction $T$. Define:
\begin{itemize}
\item Training: $\rho_{\text{train}} \ll 0$ (near-Leontief complementarity), $T_{\text{cent}} \approx 0$ (datacenter synchronization), $T_{\text{dist}} \gg T^*(\rho_{\text{train}})$ (distributed latency).
\item Inference: $\rho_{\text{inf}} \approx 1$ (independent queries), so $K_{\text{inf}} \approx 0$.
\end{itemize}
Then:
\begin{enumerate}[label=(\alph*)]
\item $K_{\text{eff}}^{\text{cent}}(\text{training}) = K_{\text{train}} \gg 0$ but $K_{\text{eff}}^{\text{dist}}(\text{training}) = 0$. Centralized training strictly dominates distributed.
\item $K_{\text{eff}}^{\text{cent}}(\text{inference}) \approx K_{\text{eff}}^{\text{dist}}(\text{inference}) \approx 0$. Neither form has a curvature advantage for inference; cost determines the winner.
\end{enumerate}
Consequently, inference decentralizes at cost parity while training remains centralized.
\end{proposition}

\begin{proof}
Part (a): Training is near-Leontief ($\rho \ll 0$, $K$ large) because all gradient updates must synchronize---any desynchronization destroys output quality (the ``straggler problem'' in distributed training is precisely the Leontief bottleneck). Centralized operation achieves $T_{\text{cent}} \approx 0$ via nanosecond NVLink, so $K_{\text{eff}}^{\text{cent}} = K_{\text{train}}$. Distributed operation has $T_{\text{dist}} \gg T^*$ because WiFi latency (${\sim}$ms) exceeds NVLink (${\sim}$ns) by 5--6 orders of magnitude, placing $T_{\text{dist}}/T^* \gg 1$. Therefore $K_{\text{eff}}^{\text{dist}} = K_{\text{train}} \cdot (1 - T_{\text{dist}}/T^*)^+ = 0$.

Part (b): Inference queries are independent ($\rho \approx 1$), so $K = (1-\rho)(J-1)/J \approx 0$. With $K \approx 0$, effective curvature is $\approx 0$ regardless of $T$. Neither centralized nor distributed operation has a curvature advantage; competition occurs purely on cost and latency.
\end{proof}

The proposition formalizes the ``topological'' constraint as a statement about information friction: the synchronization gap is not merely an engineering inconvenience but a fundamental $T$ barrier that makes the complementarity of training inputs unexploitable in a distributed setting. It also makes a falsifiable prediction about when the bifurcation could weaken: asynchronous training algorithms (gradient compression, local SGD, federated averaging) effectively raise the $\rho$ of training by tolerating partial desynchronization. If such methods reduce $K_{\text{train}}$ sufficiently, the temperature barrier $T^*(\rho_{\text{train}})$ rises, and $K_{\text{eff}}^{\text{dist}}$ becomes positive. The prediction is that distributed training becomes viable not when communication speeds improve (lowering $T_{\text{dist}}$) but when algorithmic tolerance for asynchrony increases (raising $\rho_{\text{train}}$).

\medskip
\noindent\textbf{Quantitative calibration of $\rho$.}
The companion paper (Smirl 2026e, Section~9.1) adopts $\rho$ at this level from Oberfield and Raval (2021), who estimate plant-level manufacturing production functions corrected for aggregation bias: $\sigma \in [0.5, 0.7]$, implying $\rho \in [-1.0, -0.43]$.  NLS estimation on FRED Computer/Electronics IP subsectors yields $\hat\rho = -0.19$ ($\sigma = 0.84$), systematically above the micro-data range as expected for macro aggregates that mix capital--labor and input--input margins.  The qualitative bifurcation---$\rho_{\text{train}} \ll 0$ (strong complementarity) versus $\rho_{\text{inf}} \approx 1$ (independence)---is robust to calibration within these ranges: the effective curvature gap $K_{\text{eff}}^{\text{cent}} - K_{\text{eff}}^{\text{dist}} \gg 0$ for training holds for any $\rho < -0.4$.

\subsection{Implications for the Model}

The effective curvature framework provides three refinements to the formal model of Section~3.

First, the continuation value $S_T$ is structurally determined rather than scenario-dependent (equation~\ref{eq:ST_structural}): it equals the quasi-rent from centralized training's curvature advantage, which persists as long as $T_{\text{dist}} \gg T_{\text{cent}}$ for training workloads.

Second, the crossing condition for inference ($R_0 > 1$ at cost parity) is validated as \emph{sufficient}: since inference has $K \approx 0$, the organizational viability condition \eqref{eq:org_viability} imposes no additional constraint beyond cost and adoption dynamics.

Third, for tasks intermediate between pure inference and pure training---federated fine-tuning ($\rho$ moderate, $K$ moderate), multi-agent evaluation ($\rho$ low-moderate), distributed RLHF ($\rho$ low)---the framework predicts a \emph{gradient of decentralization} ordered by $\rho$. Tasks decentralize in sequence from highest $\rho$ (inference, first) to lowest (training, last or never), as each task's $T_{\text{mesh}}$ falls below its $T^*(\rho)$.
...
This is an instance of the \emph{sequential wave} pattern formalized in the companion paper (Smirl 2026d, Theorem~4.3(v)): the semiconductor industry's first technology wave (planar die scaling, 1970s--2010s) has matured, while a second wave (3D stacking and advanced packaging, 2015--present) is opening new timescale gaps and increasing the effective hierarchy depth of the production process.
...
The training-inference bifurcation sharpens the mechanism's empirical scope. The post-crossing equilibrium is partial decentralization: inference distributes to edge devices while training persists in centralized clusters. This coexistence is stable because the architectural constraints on training are topological, not cost-based---formalized in Proposition~\ref{prop:bifurcation} as a consequence of the effective curvature theorem: training's near-Leontief complementarity ($\rho \ll 0$) is destroyed by the information friction of distributed coordination ($T_{\text{dist}} \gg T^*$), while inference's near-perfect substitutability ($\rho \approx 1$) makes it indifferent to coordination quality. The generalized crossing condition ($R_0 > 1$) endogenizes the 3--5 year coordination layer lag observed in historical transitions and predicts compression to 2--3 years for the current AI transition. The rational inattention derivation of adoption dynamics provides structural foundations for the $R_0$ parameters: the adoption rate, coordination friction, and latency premium are jointly determined by user information capacity rather than independently calibrated.

What the mechanism predicts unambiguously is that concentrated investment endogenously produces inference decentralization, that this process accelerates with the number of competitors and is amplified by asymmetric players who benefit from crossing, and that training centralization and inference decentralization will coexist as stable features of the AI economic landscape. The companion effective curvature framework further predicts a \emph{gradient of decentralization} for intermediate tasks---federated fine-tuning, multi-agent evaluation, distributed RLHF---ordered by each task's CES complementarity parameter, with inference decentralizing first and training last.
The endogenous decentralization mechanism is itself one episode in the technology lifecycle: the companion paper (Smirl 2026d) proves that the number of hierarchy levels is endogenously determined by the timescale distribution of the underlying technology and follows a lifecycle from nascent to innovating to mature.
The semiconductor industry's transition from planar die scaling (mature, $\alpha \to 0$) to 3D packaging (early-stage, $\alpha = 0.23$) is a sequential wave within this lifecycle, and the four-level AI hierarchy ($N_{\mathrm{eff}} = 4$) is the current-era instantiation of a general $N_{\mathrm{eff}}$-level structure.
...
\bibitem{matejka2015} Mat\v{e}jka, F., \& McKay, A. (2015). Rational inattention to discrete choices: A new foundation for the multinomial logit model. \emph{American Economic Review}, 105(1), 272--298.
...
\bibitem{smirl2026ces} Smirl, J. (2026a). The CES quadruple role: Superadditivity, correlation robustness, strategic independence, and network scaling as four properties of CES curvature. Working Paper.

\bibitem{smirl2026free} Smirl, J. (2026b). Free energy economics: A unified framework from CES aggregation and Tsallis entropy. Working Paper.

\bibitem{smirl2026prod} Smirl, J. (2026c). Production under information frictions: A CES potential theory of the firm. Working Paper.

\bibitem{smirl2026ch} Smirl, J. (2026d). Complementary heterogeneity in hierarchical economies. Working Paper.
