...
ln(estates\_st) = $\beta$·ln(1 - $\tau$\_st) + X\_st\textquotesingle $\gamma$ + $\mu$\_s + $\delta$\_t + $\varepsilon$\_st

where $\tau$\_st is the combined federal-state marginal tax rate.

The coefficient $\beta$ estimates the elasticity of reported estates with respect to the net-of-tax rate.
...
y\_it = $\alpha$ + $\beta$·UK\_i + X\_it\textquotesingle $\gamma$ + $\delta$\_t + $\varepsilon$\_it
...
y\_ist = $\alpha$ + $\beta$·(Post\_t × Repeal\_s) + $\mu$\_s + $\delta$\_t + X\_ist\textquotesingle $\gamma$ + $\varepsilon$\_ist

\textbf{Step 4: Event study specification}

y\_ist = $\alpha$ + $\Sigma$\_k $\beta$\_k·(Year\_t = k) × Repeal\_s + $\mu$\_s + $\delta$\_t + $\varepsilon$\_ist

Plot $\beta$\_k coefficients to visualize pre-trends and treatment effects.
...
$\square$ Download IRS SOI public-use estate tax files

$\square$ Download SCF data (2019, 2022 waves)

$\square$ Register for PSID access and create custom extract

$\square$ Download HMRC inheritance tax statistics

$\square$ Register for UK Data Service and download WAS

$\square$ Compile state inheritance tax parameters (1990-2025)

$\square$ Download Census state government finance data

$\square$ Download OECD tax revenue and wealth distribution data

$\square$ Submit application for IRS restricted data access (if pursuing)

8.2 Phase 2: Data Cleaning (Weeks 9-16)

$\square$ Harmonize US and UK wealth definitions

$\square$ Create state-year panel of tax parameters

$\square$ Link estate tax data to state characteristics

$\square$ Construct inheritance receipt variables from SCF/PSID

$\square$ Create consistent beneficiary classification across datasets

$\square$ Document all variable definitions and coding decisions

8.3 Phase 3: Estimation (Weeks 17-28)

$\square$ Estimate bequest elasticity (Section 6.1)

$\square$ Estimate planning effectiveness parameter

$\square$ Estimate dispersion preference parameter

$\square$ Run cross-country UK vs. US analysis (Section 6.2)

$\square$ Run state-level diff-in-diff (Section 6.3)

$\square$ Conduct robustness checks and sensitivity analysis

$\square$ Bootstrap standard errors

8.4 Phase 4: Write-Up (Weeks 29-36)

$\square$ Update paper with empirical results

$\square$ Create publication-quality tables and figures

$\square$ Write detailed data appendix

$\square$ Prepare replication package

$\square$ Submit to journal
