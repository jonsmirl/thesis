...
% TITLE
% -------------------------------------------------------------------
\title{\textbf{The CES Quadruple Role}\\[6pt]
\large Superadditivity, Correlation Robustness, Strategic Independence,\\
and Network Scaling as Four Properties of CES Curvature}
...
\begin{abstract}
\noindent The CES production function is ubiquitous in economics, yet four of its
important properties---superadditivity, robustness to correlated inputs,
resistance to strategic manipulation, and network scaling---have been studied
separately using different techniques.  This paper proves they are controlled by a
single parameter: the \textbf{curvature parameter} $K = (1-\rho)(J-1)/J$, derived
from the principal curvature of the CES isoquant at the cost-minimizing point.
Superadditivity gap $= \Omega(K)\cdot\text{diversity}$; correlation robustness
bonus $= \Omega(K^2)\cdot\text{idiosyncratic variation}$; strategic manipulation
penalty $= -\Omega(K)\cdot\text{deviation}^2$; network scaling exponent $=
1/\rho$.  All bounds tighten monotonically in $K$.  When $K = 0$ (perfect
substitutes), all four vanish simultaneously.  Properties (a)--(c) are the same
geometric fact---the curvature of the isoquant---viewed from aggregation theory,
information theory, and game theory.  Property (d) extends this to endogenous
network size, connecting CES complementarity to Sarnoff, Metcalfe, and
super-Metcalfe scaling.  Results extend to general (unequal) CES weights via the
secular equation of the weighted inverse-share matrix, whose smallest root
$R_{\min}$ controls the generalized curvature parameter.
\end{abstract}

\medskip
\noindent\textbf{Keywords:} CES production function, isoquant curvature,
superadditivity, diversification, strategic independence, network scaling, secular equation
...
Four important properties of CES aggregation have been established in various
settings:
...
\item \emph{Network scaling:} The unnormalized CES aggregate scales as $J^{1/\rho}$
in the number of symmetric components.  This matters for platform economics, where
the scaling exponent determines whether a network exhibits linear (Sarnoff),
quadratic (Metcalfe), or super-Metcalfe value.
\end{enumerate}
...
independence from cooperative game theory, network scaling from the definition of
the unnormalized aggregate.  This paper demonstrates that all four are controlled
...
The main result (Theorem~\ref{thm:quadruple}) states:
...
\item[(d)] The unnormalized CES aggregate scales as $J^{1/\rho}$ in the number of
symmetric components; the network scaling exponent $1/\rho$ is a monotone function
of $K$.
\end{itemize}
All four bounds tighten monotonically in $K$.  When $K = 0$ (perfect substitutes,
$\rho = 1$), all four vanish.

The underlying mechanism in cases (a)--(c) is the same: the isoquant is curved.
...
single geometric object, not three consequences of a common assumption.  Property
(d) extends this to endogenous network size: the same parameter $\rho$ that
determines isoquant curvature also determines how aggregate value scales with the
...
first three results.  Section~\ref{sec:network} proves the network scaling result.
Section~\ref{sec:unified} presents the unified perspective.
...
The components are called \emph{complements} when $\rho < 0$ ($\sigma < 1$)
...
Both properties are standard and are used freely throughout.
\end{remark}

Sections~\ref{sec:curvature}--\ref{sec:unified} work with
...
The object of interest is the aggregate $Y = F(\bX)$.
...
The first three properties concern the \emph{normalized} CES aggregate $F = (\frac{1}{J}\sum x_j^\rho)^{1/\rho}$, which is homogeneous of degree~1 and bounded by construction.  The fourth property concerns the \emph{unnormalized} aggregate, which reveals how total value scales with network size.

\subsection{The Unnormalized CES Aggregate}
...
G(J) = J^{1/\rho} \cdot c.
\end{equation}
The network scaling exponent is $1/\rho$, which is a monotone function of
$K = (1-\rho)(J-1)/J$.  The three regimes are:
\begin{enumerate}[label=(\roman*)]
...
\end{enumerate}
As $\rho \to 0$ (Cobb-Douglas): the scaling becomes exponential
($\log G/\log J \to \infty$).  For $\rho < 0$ (strict complements):
$1/\rho < 0$, so $G \to 0$ as $J \to \infty$---adding components
\emph{dilutes} value, because each additional complement creates a new
potential bottleneck.
\end{theorem}

\begin{proof}
At the symmetric point $x_j = c$:
$G = (\sum_{j=1}^J c^\rho)^{1/\rho} = (J \cdot c^\rho)^{1/\rho} = J^{1/\rho} \cdot c$.
The exponent $1/\rho$ is a decreasing function of $\rho$ for $\rho > 0$
and hence an increasing function of $K$ (since $K$ is decreasing in $\rho$).
The regime classification follows from evaluating $1/\rho$ at the stated values.
\end{proof}

\begin{remark}[Economic content]
The network scaling exponent $1/\rho$ connects the production technology to
the platform economics literature.  In a network where each of $J$ nodes
contributes equally and the aggregate value is CES:
\begin{itemize}[leftmargin=2em]
\item Sarnoff networks ($\rho = 1$, perfect substitutes): each additional node
adds exactly $c$ units of value.  Broadcast media.
\item Metcalfe networks ($\rho = 1/2$): value grows as $J^2$.  Communication
networks where pairwise connections matter.
\item Super-Metcalfe networks ($\rho < 1/2$): value grows faster than $J^2$.
Networks with group-level complementarities (e.g., AI model ecosystems where
each specialized model makes all others more valuable).
\end{itemize}
The CES parameter $\rho$ thus determines not only whether inputs are
complements or substitutes in production, but also whether networks exhibit
weak or strong scaling effects.  This is not a coincidence: both are
consequences of how the aggregate responds to adding a new symmetric
component, which is geometrically determined by isoquant curvature.
\end{remark}

\begin{remark}[Relationship to (a)--(c)]
Properties (a)--(c) are three views of the same geometric fact: the
curvature of the CES isoquant at a fixed number of components $J$.
Property (d) holds $J$ variable and asks how the aggregate scales.  The
connection is that $1/\rho = \sigma$ (the elasticity of substitution) and
$K = (1-\rho)(J-1)/J$, so the network exponent and the curvature
parameter are both determined by $\rho$.  Higher complementarity (lower
$\rho$, higher $K$) simultaneously strengthens all four properties: larger
superadditivity gaps, more robust diversification, stronger strategic
stability, and faster-growing networks.
\end{remark}


% ===================================================================
% 8. THE UNIFIED PERSPECTIVE
% ===================================================================
...
\noindent\textbf{(d) Network scaling}
(Theorem~\ref{thm:network}).  Unnormalized aggregate:
\[
G(J) = J^{1/\rho}\cdot c.
\]
The exponent $1/\rho$ is monotone in $K$: linear at $K = 0$, Metcalfe at $K = (J-1)/(2J)$, super-Metcalfe for larger $K$.

\medskip\noindent All four properties tighten monotonically in $K$.
\end{theorem}

\subsection{The Geometric Intuition}

The four properties share a common origin: \textbf{the isoquant is not flat.}
...
flat surface.  All four properties vanish:
gap $= 0$, curvature bonus $= 0$, manipulation penalty $= 0$, scaling
exponent $= 1$ (linear).

\medskip
\noindent\textbf{For CES with $\rho < 1$} ($K > 0$):
$\mathcal{I}_1$ curves toward the origin.  The curvature has four
...
\item \textbf{Network scaling.}  Adding a new symmetric component
to the unnormalized aggregate increases total value by a factor
$((J+1)/J)^{1/\rho}$, which exceeds $(J+1)/J$ (the linear baseline)
whenever $\rho < 1$.  The curvature that makes diverse inputs
super-additive in production also makes each additional network
participant super-linearly valuable.
\end{enumerate}
...
O(K^2)$.  Property (d) is different: the scaling exponent $1/\rho$
depends on $\rho$ directly (not through the Hessian), so it is a
zeroth-order consequence of the functional form---but the same
parameter $\rho$ that determines isoquant curvature also determines
network scaling.

This is consistent: (a) and (c) ask ``how much does $F$ change?''
(first derivative of curvature).  Part (b) asks ``how much does
$F$ vary?'' (second derivative of curvature).  Part (d) asks ``how
does the aggregate grow with $J$?'' (a global property of the
functional form).  The information channel is the square of the
curvature channel; the scaling channel is the zeroth-order channel.
...
\subsection{General-Weight Versions of the Four Theorems}
...
\medskip
\noindent\textbf{(d) Network scaling.}
The unnormalized aggregate $G(\bx) = (\sum a_j x_j^\rho)^{1/\rho}$ at the
cost-minimizing point satisfies $G = \Phi^{1/\rho} \cdot c$, where
$\Phi = \sum p_j$ depends on the weight vector.  For $J$ equal-weight components,
this reduces to $G = J^{1/\rho} \cdot c$ as in Theorem~\ref{thm:network}.  For
general weights, the effective network size is $\Phi^{1/\rho}$, which accounts for
...
(d) At the cost-minimizing point, $G = (\sum a_j (x_j^*)^\rho)^{1/\rho}
= (\sum a_j (c\,p_j/\Phi^{1/\rho})^\rho)^{1/\rho} = \Phi^{1/\rho}\cdot c$
by direct substitution of~\eqref{eq:xstar}.
\end{proof}
...
$K(\rho, \ba)$.  This $K$ then enters all four bounds.  The
...
This paper proves static properties of a single CES aggregate.\footnote{The quadruple role is not four properties of an assumed functional form: CES is the unique aggregation function compatible with constant returns to scale and scale consistency (aggregation invariant to partitioning).  See the companion paper \cite{smirl2026emergent} for the proof, which shows that CES is the fixed point of the aggregation renormalization group---the production-theory analogue of the Central Limit Theorem.}  It says
nothing about:
\begin{itemize}[leftmargin=2em]
...
\bibitem{smirl2026emergent}
Smirl, J. (2026).
Emergent CES: Why constant elasticity of substitution is not an assumption.
Working Paper.
