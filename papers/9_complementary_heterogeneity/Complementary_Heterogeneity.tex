...
Taking the CES Triple Role (Smirl~2026a) as given---within each sector, the curvature parameter $K = (1-\rho)(J-1)/J$ simultaneously controls diversity gains, informational robustness, and resistance to manipulation---the paper asks what happens \emph{between} sectors.
...
The CES Triple Role (Smirl~2026a, henceforth ``Paper~A'') guarantees that within each sector, diversity is productive (superadditivity gap $\propto K$), informationally robust (effective dimensionality bonus $\propto K^2$), and resistant to strategic manipulation (penalty $\propto K$).
...
Section~\ref{sec:setup} establishes the hierarchical economy and the CES free energy.
...
Section~\ref{sec:levels} applies the framework to the four economic levels.
...
\subsection{The Free Energy}\label{sec:freeenergy}
...
within-sector eigenstructure, coupling topology (aggregate, directed, nearest-neighbor), existence of an activation threshold, the Triple Role (from Paper~A), and the Eigenstructure Bridge (Section~\ref{sec:bridge}).
...
On the slow manifold---where the fast sector equilibrates before the slow sector moves appreciably (Theorem~\ref{thm:topology}(i))---the $NJ$-dimensional system reduces to $N$ scalar equations.
...
\begin{proof}
By Theorem~\ref{thm:topology}(i), the slow manifold at each level is parameterized by $F_n$.
...
For each level, we present the economic content, the gain function $\phi_n$, the relevant CES property from the Triple Role (Paper~A), and the ceiling from the slow manifold cascade.
...
Where $K$ enters: $K$ controls the \emph{sharpness} of the transition (how quickly the economy accelerates once it begins transitioning), not the \emph{duration}.
...
\noindent\textbf{P1--P3: Testing the Triple Role (Paper~A, applied at each level).}
...
General weights yield results via the secular equation (Paper~A, Section~8), but bounds are less clean.
...
The term $\Psi'/\Psi$ propagates through the entire cascade of ceiling functions, with the CES elasticity entering at the capability level.

\subsection{Where $K$ Enters}

The second-order coefficient $b = \frac{1}{2}\partial^2 g/\partial F_1^2|_{\text{bif}}$ contains $\partial^2 \FCES/\partial F_2^2|_{\text{sym}} = -K/(J\bar{F}_2)$ (from the CES Hessian restricted to the aggregate direction).
