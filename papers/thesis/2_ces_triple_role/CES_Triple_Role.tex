...
% 1. INTRODUCTION
% ===================================================================
\section{Introduction}\label{ces:sec:intro}

The constant elasticity of substitution (CES) production function, introduced by
Arrow, Chenery, Minhas, and Solow~\cite{ces:arrow1961}, is among the most widely used
...
construction.  The Dixit-Stiglitz~\cite{ces:dixit1977} formulation of monopolistic
competition, the Jones~\cite{ces:jones2005} analysis of directed technical change, and
the Houthakker~\cite{ces:houthakker1955} aggregation results all rest on CES.
...
\begin{equation}\label{ces:eq:K_intro}
K \;=\; (1-\rho)\,\frac{J-1}{J}
\end{equation}
...
The main result (Theorem~\ref{ces:thm:triple}) states:
...
parameter $K(\rho, \ba)$ that replaces~\eqref{ces:eq:K_intro} in all three bounds.

Section~\ref{ces:sec:setup} establishes notation.  Section~\ref{ces:sec:curvature} proves
the Curvature Lemma.  Sections~\ref{ces:sec:superadd}--\ref{ces:sec:strategic} prove the
three results.  Section~\ref{ces:sec:unified} presents the unified perspective.
Section~\ref{ces:sec:general} extends everything to general weights.
Section~\ref{ces:sec:discussion} discusses tightness, prior results, and applications.


% ===================================================================
% 2. SETUP AND NOTATION
% ===================================================================
\section{Setup and Notation}\label{ces:sec:setup}

\subsection{The CES Aggregate}

For $J \geq 2$ components, the \textbf{CES aggregate} with weights
$a_j > 0$ summing to 1 is
\begin{equation}\label{ces:eq:CES}
...
For Sections~\ref{ces:sec:curvature}--\ref{ces:sec:unified} we work with
\textbf{equal weights} $a_j = 1/J$.  Section~\ref{ces:sec:general} presents
...
% ===================================================================
\section{The Curvature Lemma}\label{ces:sec:curvature}

This section establishes the geometric foundation.  The three economic
results of Sections~\ref{ces:sec:superadd}--\ref{ces:sec:strategic} all follow
from the eigenstructure derived here.

\subsection{Gradient at the Symmetric Point}

\begin{proposition}[Equal marginal products]\label{ces:prop:gradient}
At the symmetric point $\bar{\bx} = c\,\bone$, the gradient of $F$ is
\begin{equation}\label{ces:eq:gradient}
...
\begin{proposition}[CES Hessian]\label{ces:prop:hessian}
The Hessian of $F$ at the symmetric point $\bar{\bx} = c\,\bone$ is
\begin{equation}\label{ces:eq:hessian}
...
\begin{definition}[Curvature parameter]\label{ces:def:K}
The \textbf{curvature parameter} of the equal-weight CES aggregate with
$J$ components is
\begin{equation}\label{ces:eq:K}
K = (1-\rho)\,\frac{J-1}{J}.
\end{equation}
\end{definition}
...
{\|\nabla F\|\cdot\|\bv\|^2}.
\]
By Proposition~\ref{ces:prop:hessian}, $\bv^T \nabla^2 F\,\bv =
-(1-\rho)/(Jc) \cdot \|\bv\|^2$ for any $\bv \in \bone^{\perp}$.  By
Proposition~\ref{ces:prop:gradient}, $\|\nabla F\| = \|\bone/J\| =
1/\sqrt{J}$.  Therefore
\[
...
% 4. SUPERADDITIVITY
% ===================================================================
\section{Superadditivity}\label{ces:sec:superadd}

\begin{theorem}[Superadditivity]\label{ces:thm:superadd}
For all $\bx, \by \in \R_+^J \setminus \{\mathbf{0}\}$:
\begin{equation}\label{ces:eq:superadd}
...
point:
\begin{equation}\label{ces:eq:supergap}
F(\bx + \by) - F(\bx) - F(\by)
\;\geq\;
...
Lemma~\ref{ces:lem:curvature}, the isoquant has uniform positive curvature
$\kappa^* = K\sqrt{J}/[c(J-1)]$ at the symmetric point.  The standard
curvature comparison for convex hypersurfaces (cf.\ do
Carmo~\cite{ces:docarmo1992}, applied to 2-plane sections through the
...
% ===================================================================
\section{Correlation Robustness}\label{ces:sec:corrrobust}
...
\begin{definition}[Effective dimension]\label{ces:def:deff}
...
\begin{theorem}[Correlation robustness]\label{ces:thm:corrrobust}
To second order in $\gamma = \tau/c$:
\begin{equation}\label{ces:eq:deff}
d_{\mathrm{eff}} \;\geq\;
...
Proposition~\ref{ces:prop:hessian}, for any $\beps = \bar{\epsilon}\,\bone
+ \bfeta$:
\begin{align*}
...
normalization yields the stated bound~\eqref{ces:eq:deff}.
\end{proof}

\subsection{The Correlation Threshold}

\begin{corollary}\label{ces:cor:threshold}
The effective dimension satisfies $d_{\mathrm{eff}} \geq J/2$ provided
\begin{equation}\label{ces:eq:rbar}
...
The superadditivity gap (Section~\ref{ces:sec:superadd}) and the strategic
manipulation penalty (Section~\ref{ces:sec:strategic}) are first-order
...
% ===================================================================
\section{Strategic Independence}\label{ces:sec:strategic}
...
\begin{definition}[Manipulation gain]\label{ces:def:manipgain}
...
\begin{theorem}[Strategic independence]\label{ces:thm:strategic}
...
\begin{equation}\label{ces:eq:strategic}
...
(Shapley~\cite{ces:shapley1971}), since $F$ is concave.  The Shapley value
...
\]
From Proposition~\ref{ces:prop:hessian}, for any $\bdelta$ with
...
Shapley's~\cite{ces:shapley1971} theory of convex games but provides
...
% ===================================================================
\section{The Unified Theorem}\label{ces:sec:unified}

\begin{theorem}[CES Triple Role]\label{ces:thm:triple}
Let $F$ be a CES aggregate~\eqref{ces:eq:CES} with equal weights,
$\rho < 1$, and $J \geq 2$.  Define $K = (1-\rho)(J-1)/J$.  Then
$K > 0$, and:

\medskip
\noindent\textbf{(a) Superadditivity}
(Theorem~\ref{ces:thm:superadd}). $F(\bx+\by) \geq F(\bx)+F(\by)$, with
gap:
\[
...
(Theorem~\ref{ces:thm:corrrobust}).  Effective dimension:
\[
...
(Theorem~\ref{ces:thm:strategic}).  Manipulation gain:
\[
...
Shapley's~\cite{ces:shapley1971} convex game theory provides the qualitative
...
% 8. GENERAL WEIGHTS
% ===================================================================
\section{General Weights and the Secular Equation}\label{ces:sec:general}
...
\begin{equation}\label{ces:eq:pj}
...
\begin{equation}\label{ces:eq:xstar}
...
\begin{proposition}\label{ces:prop:gen_hessian}
...
\end{equation}
In matrix form:
\begin{equation}\label{ces:eq:gen_hessian}
\nabla^2 F\big|_{\bx^*}
...
\begin{proposition}[Secular equation]\label{ces:prop:secular}
...
\begin{equation}\label{ces:eq:secular}
...
\begin{proof}
The Hessian~\eqref{ces:eq:gen_hessian} restricted to $\bone^{\perp}$ (the
...
the secular equation~\eqref{ces:eq:secular}.  This is a standard result
...
\begin{definition}\label{ces:def:genK}
The \textbf{generalized curvature parameter} is
\begin{equation}\label{ces:eq:genK}
K(\rho, \ba) = (1-\rho)\,\frac{J-1}{J}\,\Phi^{1/\rho}\,R_{\min}
\end{equation}
where $R_{\min} = \mu_1$ is the smallest root of the secular
equation~\eqref{ces:eq:secular}.
\end{definition}

\begin{proposition}\label{ces:prop:K_reduction}
...
\begin{theorem}[General-weight Triple Role]\label{ces:thm:gen_triple}
Let $F$ be a CES aggregate with weights $\ba$ and generalized curvature
parameter $K(\rho, \ba)$ from~\eqref{ces:eq:genK}.  Then:
...
\begin{equation}\label{ces:eq:KS}
...
Proposition~\ref{ces:prop:secular}.

(b) The expansion of $F(\bX)$ around $\bx^*$ uses the general
Hessian~\eqref{ces:eq:gen_hessian}.  The spectral decomposition of the
...
% 9. DISCUSSION
% ===================================================================
\section{Discussion}\label{ces:sec:discussion}
...
\emph{International trade.}  The Dixit-Stiglitz~\cite{ces:dixit1977}
...
\emph{Directed technical change.}  Jones~\cite{ces:jones2005} studies how
