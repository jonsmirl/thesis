...
This paper develops a theory of hierarchical economies with CES production at each level, operating across four timescales: hardware (decades), network formation (years), capability accumulation (months), and financial settlement (days).

Taking the CES Triple Role~\cite{ch:smirl2026a} as given---within each sector, the curvature parameter $K = (1-\rho)(J-1)/J$ simultaneously controls diversity gains, informational robustness, and resistance to manipulation---the paper asks what happens \emph{between} sectors.
...
% 1. INTRODUCTION
% -------------------------------------------------------------------
\section{Introduction}\label{ch:sec:intro}
...
Each sector aggregates heterogeneous inputs using CES production technology.
The CES Triple Role~\cite{ch:smirl2026a} (henceforth ``Paper~A'') guarantees that within each sector, diversity is productive (superadditivity gap $\propto K$), informationally robust (effective dimensionality bonus $\propto K^2$), and resistant to strategic manipulation (penalty $\propto K$).
...
\medskip
\noindent\textbf{Result 1: Derived architecture} (Section~\ref{ch:sec:topology}).
...
\medskip
\noindent\textbf{Result 2: Activation threshold} (Section~\ref{ch:sec:threshold}).
...
\medskip
\noindent\textbf{Result 3: Hierarchical ceiling} (Section~\ref{ch:sec:transition}).
...
The Baumol cost disease and the Triffin dilemma are the same mathematical object---a slow-manifold constraint---at adjacent layers in the hierarchy.

\medskip
\noindent\textbf{Result 4: Welfare decomposition} (Section~\ref{ch:sec:welfare}).
...
\medskip
\noindent\textbf{Result 5: Damping cancellation} (Section~\ref{ch:sec:damping}).
...
At current semiconductor improvement rates ($\approx 15\%$ annual cost decline), this yields an approximately 8-year transition window (Section~\ref{ch:sec:transition}).

The paper proceeds as follows.
Section~\ref{ch:sec:setup} establishes the hierarchical economy and the CES free energy.
Section~\ref{ch:sec:topology} proves the Port Topology Theorem and the Moduli Space characterization.
Section~\ref{ch:sec:threshold} derives the next-generation matrix and the spectral activation threshold.
Section~\ref{ch:sec:levels} applies the framework to the four economic levels.
Section~\ref{ch:sec:bridge} proves the Eigenstructure Bridge and welfare decomposition.
Section~\ref{ch:sec:damping} proves the damping cancellation and derives the upstream reform principle.
Section~\ref{ch:sec:transition} derives the hierarchical ceiling cascade and the transition duration.
Section~\ref{ch:sec:predictions} presents eleven empirical predictions.
Section~\ref{ch:sec:limitations} states limitations.
Section~\ref{ch:sec:conclusion} concludes.


% -------------------------------------------------------------------
% 2. SETUP
% -------------------------------------------------------------------
\section{Setup}\label{ch:sec:setup}

\subsection{The CES Aggregate}\label{ch:sec:ces}

For $J \geq 2$ components at level $n$ of an $N$-level hierarchy, the \textbf{CES aggregate} with equal weights is
\begin{equation}\label{ch:eq:CES}
...
By Paper~A~\cite{ch:smirl2026a}, $K$ simultaneously controls three within-sector properties:
...
We do not reprove them here; see Paper~A~\cite{ch:smirl2026a} for the complete development.

\subsection{The Hierarchical Economy}\label{ch:sec:hierarchy}

The dynamics of the hierarchical economy are
\begin{equation}\label{ch:eq:dynamics}
...
\begin{itemize}[leftmargin=2em]
\item $\varepsilon_n > 0$: characteristic adjustment speed of sector~$n$. Hardware adjusts over decades ($\varepsilon_1 = 1$); financial settlement adjusts over days ($\varepsilon_4 \ll 1$).
...
\begin{table}[htbp]
\centering
\caption{Derived objects and notation.}\label{ch:tab:notation}
\begin{tabular}{lll}
\toprule
...
$\Phi$ & $-\sum_n \log F_n$ & Production difficulty (free energy) \\
...
(2)~$J \geq 2$ components per level, $N \geq 2$ levels: nontrivial diversity and nontrivial hierarchy;
(3)~timescale separation $\varepsilon_1 \gg \varepsilon_2 \gg \cdots \gg \varepsilon_N$: hardware adjusts much slower than finance (empirically uncontroversial);
...
\subsection{The Free Energy}\label{ch:sec:freeenergy}

Define the \textbf{production difficulty} (free energy) as
\begin{equation}\label{ch:eq:Phi}
...
At the symmetric allocation $x_{nj} = \bar{x}$ for all~$j$, the Hessian of the CES aggregate is (Paper~A~\cite{ch:smirl2026a}, Curvature Lemma):
\begin{equation}\label{ch:eq:hessian}
...
\begin{equation}\label{ch:eq:jacobian}
...
% -------------------------------------------------------------------
\section{The Port Topology Theorem}\label{ch:sec:topology}
...
\begin{theorem}[CES-Forced Topology]\label{ch:thm:topology}
Under the standing assumptions of Section~\ref{ch:sec:hierarchy}, the hierarchical CES economy has the following topological properties:
...
\noindent\textit{Proof sketches follow for each claim. Full proofs are in Appendix~\ref{ch:app:topology}.}
...
Second, the diversity modes decay $(2-\rho)$ times faster than the aggregate mode, creating a spectral gap (Equation~\ref{ch:eq:jacobian}).
...
Consider a two-sector system on the slow manifold (scalar dynamics per sector).
...
Since $\nabla F_n = (1/J)\,\mathbf{1}$ at the symmetric point (Paper~A~\cite{ch:smirl2026a}, Proposition~2.1), the natural CES-compatible coupling direction is the gradient.
...
Level~1, being fastest ($\varepsilon_1 \ll \varepsilon_2$), equilibrates to $F_1^* = \beta_1/(\sigma_1 J)$, a constant on the slow manifold.
...
\subsection{The Moduli Space Theorem}\label{ch:sec:moduli}

\begin{theorem}[Structural Determination]\label{ch:thm:moduli}
Fix $\rho < 1$ and the structural integers $(J, N)$.
Then:

\emph{Qualitative invariants} (determined by $\rho$):
within-sector eigenstructure, coupling topology (aggregate, directed, nearest-neighbor), existence of an activation threshold, the Triple Role (from Paper~A~\cite{ch:smirl2026a}), and the Eigenstructure Bridge (Section~\ref{ch:sec:bridge}).
...
\subsection{The Secular Equation and the Value of Diversity}\label{ch:sec:secular}

The equal-weight case ($a_j = 1/J$) used throughout the paper is analytically clean but empirically restrictive.
Paper~A~\cite{ch:smirl2026a} (Section~8) develops the general-weight theory in full.
The key objects are summarized here because they control the quantitative strength of every result in this paper.

\begin{proposition}[Secular equation, Paper~A~\cite{ch:smirl2026a}]\label{ch:prop:secular_summary}
...
\begin{equation}\label{ch:eq:secular}
...
\begin{equation}\label{ch:eq:genK}
...
Every quantitative result in this paper---the superadditivity gap (Section~\ref{ch:sec:ces}), the spectral threshold (Section~\ref{ch:sec:threshold}), the welfare weights (Section~\ref{ch:sec:bridge}), the manipulation penalty---depends on $K$.
Under general weights, $K$ is replaced by $K(\rho, \mathbf{a})$ from Equation~\eqref{ch:eq:genK}, and $R_{\min}$ enters as a known function of the weight vector through the secular equation.
...
This single number then enters all three parts of the Triple Role: superadditivity ($\propto K$), correlation robustness ($\propto K^2$), and strategic independence ($\propto K$).


% -------------------------------------------------------------------
% 4. THE REDUCED SYSTEM AND ACTIVATION THRESHOLD
% -------------------------------------------------------------------
\section{The Reduced System and Activation Threshold}\label{ch:sec:threshold}

\subsection{Reduction to $N$ Dimensions}\label{ch:sec:reduction}

On the slow manifold---where the fast sector equilibrates before the slow sector moves appreciably (Theorem~\ref{ch:thm:topology}(i))---the $NJ$-dimensional system reduces to $N$ scalar equations.

\begin{proposition}[Reduced dynamics]\label{ch:prop:reduced}
On the slow manifold, the aggregate dynamics are
\begin{equation}\label{ch:eq:reduced}
\varepsilon_n \dot{F}_n = \phi_n(F_{n-1})/J - \sigma_n F_n, \qquad n = 1, \ldots, N.
\end{equation}
\end{proposition}

\begin{proof}
By Theorem~\ref{ch:thm:topology}(i), the slow manifold at each level is parameterized by $F_n$.
Project the dynamics onto the aggregate: $\dot{F}_n = \nabla F_n \cdot \dot{\mathbf{x}}_n = (1/J)\mathbf{1} \cdot \dot{\mathbf{x}}_n$.
Summing~\eqref{ch:eq:dynamics} over $j$: $\varepsilon_n J \dot{F}_n = T_n - \sigma_n J F_n$, using $\sum_j \partial F_n/\partial x_{nj} = 1$ (Euler's theorem for degree-1 homogeneous functions) and $\sum_j x_{nj} = J F_n$ (symmetric allocation).
...
This reduction is not an approximation---it is exact on the slow manifold, with $O(\varepsilon)$ corrections from the fast dynamics.

\subsection{The Next-Generation Matrix}\label{ch:sec:NGM}
...
\begin{equation}\label{ch:eq:NGM}
\mathbf{K} = -T\Sigma^{-1}.
\end{equation}
...
\subsection{Characteristic Polynomial}\label{ch:sec:charpoly}

\begin{theorem}[NGM characteristic polynomial]\label{ch:thm:charpoly}
...
\begin{equation}\label{ch:eq:charpoly}
...
\subsection{The Spectral Threshold}\label{ch:sec:spectral}

\begin{theorem}[Activation threshold]\label{ch:thm:activation}
...
% -------------------------------------------------------------------
\section{The Four Economic Levels}\label{ch:sec:levels}

This section applies the framework to each of the four levels.
For each level, we present the economic content, the gain function $\phi_n$, the relevant CES property from the Triple Role (Paper~A~\cite{ch:smirl2026a}), and the ceiling from the slow manifold cascade.

\begin{table}[htbp]
\centering
\caption{Timescale hierarchy.}\label{ch:tab:timescales}
\begin{tabular}{llll}
\toprule
\textbf{Level} & \textbf{Domain} & \textbf{Timescale} & \textbf{Ordering} \\
\midrule
$n=1$ (slowest) & Hardware (learning curves) & $\varepsilon_1 = 1$ & Reference \\
$n=2$ & Network (mesh formation) & $\varepsilon_2$ & $\varepsilon_2 \ll 1$ \\
...
\subsection{Hardware (Level 1, Slowest---Decades)}\label{ch:sec:hardware}
...
\subsection{Network (Level 2---Years)}\label{ch:sec:network}
...
\begin{equation}\label{ch:eq:adoption}
...
The CES superadditivity (Paper~A~\cite{ch:smirl2026a}, Theorem~3.1) quantifies the diversity premium: combining agents with different capability profiles produces more aggregate capability than the sum of their individual contributions.
...
By Theorem~\ref{ch:thm:topology}(i), $F_n$ is a sufficient statistic for the level's state.
...
\subsection{Capability (Level 3---Months)}\label{ch:sec:capability}
...
The CES correlation robustness (Paper~A~\cite{ch:smirl2026a}, Theorem~3.2) provides collapse protection: the diversity of the training data prevents model collapse \cite{ch:shumailov2024}.
...
\subsection{Settlement (Level 4, Fastest---Days)}\label{ch:sec:settlement}
...
The CES strategic independence (Paper~A~\cite{ch:smirl2026a}, Theorem~3.3) makes manipulation unprofitable: diversity modes decay $(2-\rho)$ times faster than aggregate modes, suppressing manipulation signals faster than legitimate price signals.
...
This is the same mathematical object as the Baumol bottleneck at Level~3---a slow manifold constraint at an adjacent layer.
...
% -------------------------------------------------------------------
\section{The Eigenstructure Bridge and Welfare Decomposition}\label{ch:sec:bridge}

\subsection{The Non-Gradient Obstruction}\label{ch:sec:nongradient}

\begin{proposition}[Non-gradient structure]\label{ch:prop:nongradient}
The hierarchical CES economy is \emph{not} a gradient flow.
...
\subsection{The Storage Function}\label{ch:sec:storage}

\begin{theorem}[Welfare distance function]\label{ch:thm:lyapunov}
Define
\begin{equation}\label{ch:eq:lyapunov}
...
The cross-level contributions cancel by the tree condition on $c_n$---this is the Li-Shuai-van den Driessche~\cite{ch:li2010} construction applied to the cycle-graph topology.
The specific coefficients $c_n = \Pcycle/k_{n,n-1}$ are those required for cancellation on the cycle graph.
See Appendix~\ref{ch:app:lyapunov} for the full proof.
\end{proof}
...
\subsection{The Bridge Equation}\label{ch:sec:bridgeeq}

\begin{theorem}[Eigenstructure Bridge]\label{ch:thm:bridge}
On the slow manifold:
\begin{equation}\label{ch:eq:bridge}
\nabla^2\Phi\big|_{\text{slow}} = W^{-1} \cdot \nabla^2 V
\end{equation}
where $W = \diag(W_{11}, \ldots, W_{NN})$ is the \textbf{institutional quality matrix} with entries
\begin{equation}\label{ch:eq:Wnn}
...
See Appendix~\ref{ch:app:bridge} for the full derivation.
\end{proof}
...
\subsection{Welfare Loss Decomposition}\label{ch:sec:welfare}

\begin{proposition}[Closed-form welfare loss]\label{ch:prop:welfare}
...
\begin{equation}\label{ch:eq:welfare}
...
\begin{equation}\label{ch:eq:welfare_uniform}
...
\subsection{The Logistic Fragility Condition}\label{ch:sec:logistic}
...
\begin{proposition}[Logistic fragility]\label{ch:prop:logistic}
...
\begin{equation}\label{ch:eq:logistic_elast}
\varepsilon_{T_n} = \frac{1 - 2u_n}{1 - u_n}.
\end{equation}
The tree coefficient has a pole at $u_n = 1/2$:
\begin{equation}\label{ch:eq:logistic_cn}
...
% -------------------------------------------------------------------
\section{The Damping Cancellation and Policy}\label{ch:sec:damping}

\subsection{The Damping-Speed Tradeoff}\label{ch:sec:tradeoff}

\begin{proposition}[Damping cancellation]\label{ch:prop:damping}
...
\begin{equation}\label{ch:eq:dissipation}
...
\begin{proof}
(i)~The eigenvalue of the reduced Jacobian is $-\sigma_n/\varepsilon_n$ (Equation~\ref{ch:eq:jacobian} restricted to the aggregate mode).
(ii)~Direct from the equilibrium condition.
(iii)~$\dot{V}_n = -c_n\sigma_n(\delta F_n)^2/\bar{F}_n$.
Substituting $c_n$ from Proposition~\ref{ch:prop:welfare}: $c_n\sigma_n = \Pcycle\sigma_{n-1}/(\beta_n J\bar{F}_n)$, which is independent of $\sigma_n$.
\end{proof}
...
\subsection{The Upstream Reform Principle}\label{ch:sec:upstream}

\begin{theorem}[Upstream reform principle]\label{ch:thm:upstream}
...
\begin{corollary}[Zero welfare effect of stablecoin regulation]\label{ch:cor:stablecoin}
...
\subsection{The Global Welfare Ordering}\label{ch:sec:ordering}

\begin{corollary}[Welfare ordering]\label{ch:cor:ordering}
...
\subsection{The Rigidity Ordering}\label{ch:sec:rigidity}
...
% -------------------------------------------------------------------
\section{Transition Dynamics}\label{ch:sec:transition}

\subsection{The Hierarchical Ceiling Cascade}\label{ch:sec:ceiling}

\begin{proposition}[Ceiling functions]\label{ch:prop:ceiling}
...
\noindent\textbf{Economic interpretation.}
The Baumol cost disease and the Triffin squeeze are the same mathematical object---a slow manifold constraint---at adjacent layers.
...
\subsection{The Transition Duration}\label{ch:sec:canard}
...
This is the standard delayed loss of stability result \cite{ch:neishtadt1987,ch:neishtadt1988,ch:berglund2006}.
See Appendix~\ref{ch:app:canard} for the full development.
\end{proof}
...
\begin{equation}\label{ch:eq:cascade_elast}
...
\begin{proposition}[Curvature dependence of the transition]\label{ch:prop:canard_K}
The second-order coefficient $b = \frac{1}{2}\partial^2 g/\partial F_1^2|_{\text{bif}}$ contains the CES second derivative
\begin{equation}\label{ch:eq:FCES_second}
\frac{\partial^2 \FCES}{\partial F_2^2}\bigg|_{\text{sym}} = -\frac{K}{J\bar{F}_2}
\end{equation}
(from the CES Hessian restricted to the aggregate direction, Paper~A~\cite{ch:smirl2026a}).
...
The economically important timing question ``how long does the transition take?'' has the answer: $O(1/\sqrt{\varepsilon_{\text{drift}}})$, with the constant controlled by the chain of gain elasticities (Equation~\ref{ch:eq:cascade_elast}), not by the CES substitution parameter.

\subsection{Dispersion as Leading Indicator}\label{ch:sec:dispersion}
...
% -------------------------------------------------------------------
\section{Empirical Predictions}\label{ch:sec:predictions}

\subsection{Calibration Inputs}\label{ch:sec:calibration}
...
\subsection{Predictions}\label{ch:sec:predlist}

\noindent\textbf{P1--P3: Testing the Triple Role (Paper~A~\cite{ch:smirl2026a}, applied at each level).}
...
% -------------------------------------------------------------------
% 9.5 PRELIMINARY EVIDENCE: PORT TOPOLOGY (CROSS-LAYER VAR)
% -------------------------------------------------------------------
\subsection{Preliminary Evidence---Port Topology}\label{ch:sec:porttest}

Theorem~\ref{ch:thm:topology}(iv) predicts nearest-neighbor coupling: the 4-level hierarchy has a tridiagonal interaction graph, so shocks at Level~$n$ propagate to Level~$n \pm 1$ but not directly to distant levels. A VAR(1) model on monthly log-differenced proxies for all four levels (2020--2025, $T = 57$) provides a preliminary test.\footnote{Scripts and replication data at \texttt{scripts/test\_cross\_layer\_var.py}. Proxies: L1 = FRED semiconductor IP index; L2 = HuggingFace ecosystem growth (milestone interpolation); L3 = inference API pricing (log-linear interpolation); L4 = DeFiLlama stablecoin market cap.}

Four tests assess the nearest-neighbor prediction:

\begin{enumerate}[leftmargin=2em]
\item \emph{Granger causality.} Of the six nearest-neighbor pairs, one is significant: L1$\to$L2 ($p = 0.058$). Of the six distant pairs, one is significant: L2$\to$L4 ($p = 0.048$). \textbf{Verdict:} ambiguous---the pattern is not cleanly tridiagonal, though the strongest link is a nearest-neighbor pair.

\item \emph{Forecast error variance decomposition (FEVD).} At $h = 12$ months, 2 of 3 non-own layers show neighbor share exceeding distant share: L2's variance is 20.0\% from neighbors versus 0.3\% distant; L3 is 14.3\% neighbor versus 3.0\% distant. L4 violates the prediction (0.5\% neighbor vs.\ 15.1\% distant). \textbf{Verdict:} partially consistent.

\item \emph{Block exogeneity} (the most directly theory-relevant test). Does L1 affect L4 conditional on L2 and L3? All three block exogeneity tests are consistent with tridiagonal topology: L1$\to$L4 ($F = 0.73$, $p = 0.40$), L4$\to$L1 ($F = 0.53$, $p = 0.47$), and L1$\to$L3 ($F = 0.04$, $p = 0.85$). The distant-layer coefficients are jointly indistinguishable from zero once intermediate layers are included. \textbf{Verdict:} consistent.

\item \emph{Impulse response sequencing.} An L1 shock should hit L2 first, then L3, then L4 with increasing delay. The observed peak timing (L1 at $h = 1$, L2 at $h = 2$, L3 and L4 at $h = 1$) is not strictly sequential. \textbf{Verdict:} not consistent, though the short sample may conflate contemporaneous effects with lagged propagation at monthly frequency.
\end{enumerate}

\textbf{Overall assessment:} the strongest test (block exogeneity) supports the tridiagonal prediction; FEVD partially supports it; Granger causality and IRF sequencing are ambiguous. The binding constraint is sample length: 57 monthly observations provide limited power for a 4-variable system. As the sample extends and higher-frequency proxies become available (particularly for L2 and L3, which currently rely on milestone interpolation), the test will sharpen.


% -------------------------------------------------------------------
% 10. LIMITATIONS
% -------------------------------------------------------------------
\section{Limitations}\label{ch:sec:limitations}

Two categories of limitations, stated without apology.

\subsection{Mathematical}\label{ch:sec:mathlimits}

\begin{enumerate}[leftmargin=2em]
\item \emph{Gain functions genuinely free.}
Theorem~\ref{ch:thm:topology}(iii) establishes this as a proved impossibility: the exponents and coefficients of $\phi_n$ are not determined by $\rho$.
...
The fast sector equilibrates before the slow sector moves appreciably (Standing Assumption~3) is required for the slow manifold to exist and for the $NJ \to N$ dimensional reduction.
Without it, the slow manifold need not exist, and the reduction is unjustified.
The nearest-neighbor topology (Theorem~\ref{ch:thm:topology}(iv)) also requires timescale separation.

\item \emph{The system is not a gradient flow.}
The lower-triangular Jacobian is a topological obstruction (Proposition~\ref{ch:prop:nongradient}).
...
Theorem~\ref{ch:thm:lyapunov} proves $\dot{V} \leq 0$, establishing local asymptotic stability.
...
General weights yield results via the secular equation (Section~\ref{ch:sec:secular}; Paper~A~\cite{ch:smirl2026a}, Section~8), with $R_{\min}$ replacing the equal-weight curvature.
...
\subsection{Empirical}\label{ch:sec:emplimits}
...
\subsection{Frameworks Considered and Rejected}\label{ch:sec:rejected}
...
The Brunnermeier-Sannikov~\cite{ch:brunnermeier2014} framework captures the same insight rigorously.
...
% 11. CONCLUSION
% -------------------------------------------------------------------
\section{Conclusion}\label{ch:sec:conclusion}
...
\begin{enumerate}[leftmargin=2em]
\item \emph{Reform upstream, not locally} (damping cancellation, Proposition~\ref{ch:prop:damping}).
Tightening regulation at any sector has zero net welfare effect.
To improve welfare at sector~$n$, reform sector~$n-1$.
\item \emph{Increase cross-sector responsiveness at any layer} (global welfare ordering, Corollary~\ref{ch:cor:ordering}).
More responsive gain functions are unambiguously welfare-improving.
\item \emph{Invest in the weakest cross-sector link} (cycle product sensitivity, Theorem~\ref{ch:thm:charpoly}).
...
% Companion paper
\bibitem{ch:smirl2026a} Smirl, J. (2026a). The CES Triple Role: Superadditivity, Correlation Robustness, and Strategic Independence as Three Views of Isoquant Curvature. Working paper.

% Core mathematical framework
\bibitem{ch:li2010} Li, M.~Y., Shuai, Z., and van den Driessche, P. (2010). Global-stability problem for coupled systems of differential equations on networks. \emph{J.\ Differential Equations} 248, 1--20.

\bibitem{ch:shuai2013} Shuai, Z., and van den Driessche, P. (2013). Global stability of infectious disease models using Lyapunov functions. \emph{SIAM J.\ Appl.\ Math.} 73, 1513--1532.

\bibitem{ch:diekmann1990} Diekmann, O., Heesterbeek, J.~A.~P., and Metz, J.~A.~J. (1990). On the definition and the computation of the basic reproduction ratio $R_0$ in models for infectious diseases in heterogeneous populations. \emph{J.\ Math.\ Biol.} 28, 365--382.

\bibitem{ch:vandendriessche2002} Van den Driessche, P., and Watmough, J. (2002). Reproduction numbers and sub-threshold endemic equilibria for compartmental models of disease transmission. \emph{Math.\ Biosci.} 180, 29--48.

\bibitem{ch:fenichel1979} Fenichel, N. (1979). Geometric singular perturbation theory for ordinary differential equations. \emph{J.\ Differential Equations} 31, 53--98.

\bibitem{ch:vanderschaft2014} van der Schaft, A., and Jeltsema, D. (2014). Port-Hamiltonian systems theory: An introductory overview. \emph{Found.\ Trends Syst.\ Control} 1(2--3), 173--378.

\bibitem{ch:docarmo1992} do Carmo, M.~P. (1992). \emph{Riemannian Geometry}. Birkh\"auser.

% Canard / delayed bifurcation
\bibitem{ch:neishtadt1987} Neishtadt, A.~I. (1987). Persistence of stability loss for dynamical bifurcations, I. \emph{Differential Equations} 23, 1385--1391.

\bibitem{ch:neishtadt1988} Neishtadt, A.~I. (1988). Persistence of stability loss for dynamical bifurcations, II. \emph{Differential Equations} 24, 171--176.

\bibitem{ch:berglund2006} Berglund, N., and Gentz, B. (2006). \emph{Noise-Induced Phenomena in Slow-Fast Dynamical Systems}. Springer.

% CES and production theory
\bibitem{ch:arrow1961} Arrow, K.~J., Chenery, H.~B., Minhas, B.~S., and Solow, R.~M. (1961). Capital-labor substitution and economic efficiency. \emph{Rev.\ Econ.\ Stat.} 43, 225--250.

\bibitem{ch:dixit1977} Dixit, A.~K., and Stiglitz, J.~E. (1977). Monopolistic competition and optimum product diversity. \emph{Amer.\ Econ.\ Rev.} 67, 297--308.

\bibitem{ch:jones2005} Jones, C.~I. (2005). The shape of production functions and the direction of technical change. \emph{Quart.\ J.\ Econ.} 120, 517--549.

\bibitem{ch:shapley1971} Shapley, L.~S. (1971). Cores of convex games. \emph{Int.\ J.\ Game Theory} 1, 11--26.

% Network science
\bibitem{ch:barabasi1999} Barab\'asi, A.-L., and Albert, R. (1999). Emergence of scaling in random networks. \emph{Science} 286, 509--512.

\bibitem{ch:pastor2001} Pastor-Satorras, R., and Vespignani, A. (2001). Epidemic spreading in scale-free networks. \emph{Phys.\ Rev.\ Lett.} 86, 3200--3203.

% Growth theory
\bibitem{ch:jones1995} Jones, C.~I. (1995). R\&D-based models of economic growth. \emph{J.\ Polit.\ Econ.} 103, 759--784.

\bibitem{ch:romer1990} Romer, P.~M. (1990). Endogenous technological change. \emph{J.\ Polit.\ Econ.} 98, S71--S102.

\bibitem{ch:aghion2018} Aghion, P., Jones, B.~F., and Jones, C.~I. (2018). Artificial intelligence and economic growth. In \emph{The Economics of Artificial Intelligence}, University of Chicago Press.

\bibitem{ch:bloom2020} Bloom, N., Jones, C.~I., Van Reenen, J., and Webb, M. (2020). Are ideas getting harder to find? \emph{Amer.\ Econ.\ Rev.} 110, 1104--1144.

\bibitem{ch:baumol1967} Baumol, W.~J. (1967). Macroeconomics of unbalanced growth. \emph{Amer.\ Econ.\ Rev.} 57, 415--426.

% Monetary economics
\bibitem{ch:brunnermeier2014} Brunnermeier, M.~K., and Sannikov, Y. (2014). A macroeconomic model with a financial sector. \emph{Amer.\ Econ.\ Rev.} 104, 379--421.

\bibitem{ch:woodford2003} Woodford, M. (2003). \emph{Interest and Prices}. Princeton University Press.

\bibitem{ch:lucas1976} Lucas, R.~E. (1976). Econometric policy evaluation: A critique. \emph{Carnegie-Rochester Conf.\ Ser.\ Public Policy} 1, 19--46.

% Dollarization
\bibitem{ch:uribe1997} Uribe, M. (1997). Hysteresis in a simple model of currency substitution. \emph{J.\ Monet.\ Econ.} 40, 185--202.

\bibitem{ch:farhi2018} Farhi, E., and Maggiori, M. (2018). A model of the international monetary system. \emph{Quart.\ J.\ Econ.} 133, 295--355.

\bibitem{ch:triffin1960} Triffin, R. (1960). \emph{Gold and the Dollar Crisis}. Yale University Press.

% Market microstructure
\bibitem{ch:kyle1985} Kyle, A.~S. (1985). Continuous auctions and insider trading. \emph{Econometrica} 53, 1315--1335.

% Autocatalysis
\bibitem{ch:hordijk2004} Hordijk, W., and Steel, M. (2004). Detecting autocatalytic, self-sustaining sets in chemical reaction systems. \emph{J.\ Theor.\ Biol.} 227, 451--461.

% Model collapse
\bibitem{ch:shumailov2024} Shumailov, I., Shumaylov, Z., Zhao, Y., Papernot, N., Anderson, R., \& Gal, Y. (2024). AI models collapse when trained on recursively generated data. \emph{Nature}, 631, 755--759.
...
\section{Proof of the Port Topology Theorem}\label{ch:app:topology}
...
By Equation~\eqref{ch:eq:jacobian}, the transverse eigenvalues (on $\mathbf{1}^{\perp}$) are $-\sigma_n(2-\rho)/\varepsilon_n$ and the tangential eigenvalue (on $\mathbf{1}$) is $-\sigma_n/\varepsilon_n$.
The spectral gap is
...
By Fenichel's geometric singular perturbation theorem~\cite{ch:fenichel1979}, the normally hyperbolic critical manifold $\mathcal{M}_n$ persists as a locally invariant slow manifold $\mathcal{M}_n^{\varepsilon}$ within $O(\varepsilon)$ of $\mathcal{M}_n$, smoothly parameterized by $F_n$.
On $\mathcal{M}_n^{\varepsilon}$, the within-level state is determined by $F_n$ up to $O(\varepsilon)$ corrections.
Consequently, $F_n$ is a sufficient statistic for level~$n$'s state on the slow manifold.
\end{proof}
...
Furthermore, $\nabla F_n = (1/J)\,\mathbf{1} \propto \mathbf{1}$ at the symmetric point (Paper~A~\cite{ch:smirl2026a}), so $\mathbf{b}_n = \nabla F_n$ is the natural CES-compatible port direction.
...
\section{Proof of the Welfare Distance Function}\label{ch:app:lyapunov}
...
This is the Li-Shuai-van den Driessche~\cite{ch:li2010} construction applied to the cycle-graph topology, using the Volterra-Lyapunov identity
...
\section{Proof of the Eigenstructure Bridge}\label{ch:app:bridge}
...
\section{Transition Dynamics: The Normal Form}\label{ch:app:canard}
...
\frac{\Psi'(\bar{F}_1)\bar{F}_1}{\Psi(\bar{F}_1)} = \varepsilon_I \cdot \varepsilon_{\bar{S}} \cdot \varepsilon_{\FCES} \cdot \varepsilon_{h_2}.
\]
This is Equation~\eqref{ch:eq:cascade_elast} in the main text.
...
\subsection{Where $K$ Enters}\label{ch:app:K_enters}

\begin{proof}[Proof of Proposition~\ref{ch:prop:canard_K}]
\emph{Step 1: CES second derivative at the symmetric point.}
From Paper~A~\cite{ch:smirl2026a}, the CES function $\FCES = \left(\frac{1}{J}\sum_{j=1}^J x_j^\rho\right)^{1/\rho}$ has second derivative along the aggregate direction $\mathbf{1}$:
\[
...
This is the standard delayed loss of stability result for passage through a structural transition \cite{ch:neishtadt1987,ch:neishtadt1988,ch:berglund2006}.
...
\section{The Welfare Loss Decomposition}\label{ch:app:welfare}

\begin{proof}[Proof of Proposition~\ref{ch:prop:welfare}]
From Theorem~\ref{ch:thm:lyapunov}, $c_n = \Pcycle/k_{n,n-1}$ where $k_{n,n-1} = \phi_n'(\bar{F}_{n-1})\bar{F}_{n-1}/|\sigma_{n-1}|$.
...
\begin{proof}[Proof of Proposition~\ref{ch:prop:damping}]
(i)~The eigenvalue of the reduced Jacobian is $-\sigma_n/\varepsilon_n$ (Equation~\ref{ch:eq:jacobian} restricted to the aggregate mode).
...
Substituting $c_n$ from Proposition~\ref{ch:prop:welfare}:
...
\begin{proof}[Proof of Corollary~\ref{ch:cor:ordering}]
...
\begin{proof}[Proof of Proposition~\ref{ch:prop:logistic}]
