...
\centering
\vspace*{2cm}
{\LARGE\bfseries THE SETTLEMENT FEEDBACK\par}
\vspace{0.8cm}
...
{February 2026\par}
\vspace{0.5cm}
{\scshape Chapter 6\par}
\vspace{1cm}
\begin{abstract}
\noindent The self-organizing AI mesh (Chapter~5) requires a programmable settlement layer for routing compensation. Dollar stablecoins, backed by US Treasuries, are the efficient settlement medium (Chapter~7). This creates a coupled dynamical system: mesh growth increases stablecoin demand, which increases Treasury demand, which improves settlement infrastructure, which accelerates mesh growth. Simultaneously, mesh agents entering capital markets transform the markets that fund and settle mesh operations. This chapter formalizes the coupling and characterizes its equilibrium structure.

Three results are new. First, I parametrize market efficiency $E(\phi)$ as a function of the fraction $\phi$ of capital managed by autonomous mesh agents. As $\phi$ increases, efficiency approaches the Grossman-Stiglitz~\cite{sf:grossman1980} limit but a residual inefficiency $\varepsilon_{\min} > 0$ is preserved as equilibrium noise. Kyle's~\cite{sf:kyle1985} price impact $\lambda$ is non-monotone in $\phi$: depth initially improves as informed volume increases, then deteriorates as noise trading exits. Second, I model the degradation of monetary policy tools---forward guidance, quantitative easing, and financial repression---as explicit functions of $\phi$ and stablecoin ecosystem size $S$, showing that each tool depends on a specific friction that mesh participation eliminates: information processing delay, arbitrage speed, and captive savings respectively. The composite monetary policy effectiveness $\mathrm{MP}(\phi, S)$ is monotonically declining in $\phi$ with a potential discontinuity in $S$ when financial repression collapses. Third, I extend the Uribe~\cite{sf:uribe1997} hysteresis model of dollarization with endogenous stablecoin access, showing that the bifurcation thresholds $\pibar(S)$ and $\pilow(S)$ are decreasing in stablecoin ecosystem size---currencies that were previously stable become vulnerable as the mesh's settlement infrastructure grows. The Farhi-Maggiori~\cite{sf:farhi2018} safety zone shrinks as mesh participation increases ($d\bbar/d\phi < 0$) because mesh agents destroy the information-insensitivity that defines safe assets (Gorton~\cite{sf:gorton2017}), while the character of instability-zone crises shifts from sunspot-driven to fundamentals-driven.
...
\vspace{0.5cm}
\noindent\textbf{Keywords:} settlement infrastructure, stablecoins, market microstructure, monetary policy effectiveness, dollarization, safe assets, Triffin dilemma, coupled dynamical systems, autonomous agents

\vspace{0.3cm}
\noindent\textbf{JEL:} E44, E52, F33, G14, G23, O33
\end{titlepage}
...
\item Mesh growth generates stablecoin demand (Chapter~5, Section~8; Chapter~5, Part~II, Section~8).
\item Mesh agents entering capital markets increase market efficiency toward the Grossman-Stiglitz~\cite{sf:grossman1980} limit.
\item Increased market efficiency degrades monetary policy tools that depend on market frictions.
\item Monetary policy degradation, combined with stablecoin access, triggers dollarization spirals in weak-currency countries through the Uribe~\cite{sf:uribe1997} hysteresis mechanism.
...
The loop has $R_0$ structure. Define $\Rsettle$ as the number of units of subsequent mesh growth produced by each unit of current mesh growth through the financial system channel. If $\Rsettle > 1$, the loop is self-reinforcing: each cycle amplifies the next. This chapter derives the conditions under which $\Rsettle > 1$ and characterizes the dynamics that follow.

Three central questions organize the analysis. First, does the coupled system admit a stable equilibrium? The Farhi-Maggiori~\cite{sf:farhi2018} framework identifies three zones for the reserve currency issuer---safety, instability, and collapse---but the zone boundaries become endogenous to mesh participation. Second, how does market efficiency change as the marginal capital market participant shifts from human to mesh agent? The Grossman-Stiglitz~\cite{sf:grossman1980} paradox structures the answer, but the transition may exhibit non-smooth features. Third, under what conditions does the dollarization spiral become self-reinforcing, and which countries are vulnerable at current stablecoin ecosystem size?
...
The activation threshold for this level follows from the general condition (Chapter~3, Theorem~4.3): the settlement feedback loop activates when the spectral radius of the next-generation matrix at Level~4 exceeds unity, $\Rsettle > 1$. The port topology theorem (Chapter~3, Theorem~3.1) constrains the coupling structure: this chapter's four-ODE system inherits the directed feed-forward topology forced by timescale separation, with aggregate coupling to the slower levels and nearest-neighbor coupling to Level~3 (capability).

By the CES Triple Role (\cite{sf:smirl2026a}, Theorem~7.1), the curvature parameter $K = (1-\rho)(J-1)/J$ (\cite{sf:smirl2026a}, Definition~3.1) simultaneously governs the superadditivity of mesh output, the correlation robustness that prevents model collapse in self-referential training, and the strategic independence that makes manipulation of settlement pricing unprofitable. These three properties are not separate assumptions invoked at different points in the analysis---they are the same geometric fact deployed at the settlement level.


% -------------------------------------------------------------------
% 2. TERMINAL CONDITIONS FROM THE PRIOR CHAPTERS
% -------------------------------------------------------------------
\section{Terminal Conditions from the Prior Chapters}
...
\subsection{From Chapter~5 (The Mesh Economy)}

Chapter~5 establishes that for $\Rz > 1$ and CES substitution parameter $\rho < 1$, there exists a finite critical mass $N^*$ such that for $N > N^*$, the mesh equilibrium exists, is unique, is locally asymptotically stable, and the mesh's collective capability exceeds centralized provision: $\Cmesh(N) > \Ccent$. At maturity, the mesh's settlement layer processes $O(N \langle k \rangle)$ inference transactions per second. Section~8 of Chapter~5 derives that the routing and compensation requirements endogenously generate demand for programmable money---the mechanism that initiates the feedback loop formalized here.

\subsection{From Chapter~7 (The Monetary Productivity Gap)}
...
\subsection{From Chapter~5, Part~II (Capability Growth)}

Chapter~5, Part~II (The Autocatalytic Mesh) removes the fixed-capability assumption and characterizes three growth regimes. The most likely near-term regime is convergence: $\Ceff(t) \to C_{\max}$ where the ceiling is determined by the Baumol bottleneck---frontier training as the non-automatable sector. Section~8 of that chapter shows that the autocatalytic loop requires training agent compensation, data marketplace transactions, and variety expansion incentives, all settled through the programmable settlement layer. The transaction volume from autocatalytic operations scales as $O(|\mathcal{R}| \cdot f \cdot N)$, where $|\mathcal{R}|$ is the size of the RAF set and $f$ is the training allocation fraction.

The key result for this chapter: as the autocatalytic core matures, settlement demand grows faster than inference demand. The binding constraint on mesh capability growth may be monetary (settlement infrastructure) rather than technological ($\phieff$, $h$, $\alpha$).

\subsection{From Chapter~4 (Endogenous Decentralization)}

Chapter~4 models concentrated AI infrastructure investment and its paradoxical consequence: the same investment that finances centralized training also finances the learning curves (3D memory stacking, advanced packaging) that enable distributed inference. The crossing point $R_0 > 1$ initiates the mesh. The rate of concentrated investment determines the frontier model improvement rate $g_Z$, which the Baumol bottleneck (Chapter~5, Part~II) identifies as the mesh's long-run growth rate limiter.

\subsection{Key Retained Assumptions}

Three assumptions carry forward from the preceding chapters:

\begin{enumerate}[label=(\roman*)]
\item \emph{Training persistence:} Frontier model training remains centralized. The mesh fine-tunes and adapts base models; it does not train frontier models from scratch. This is the exogenous input (``food set'') whose growth rate $g_Z$ bounds the mesh through the Baumol bottleneck.

\item \emph{CES structure:} The aggregate capability function retains the CES form $\Ceff = (\sum_j C_j^\rho)^{1/\rho}$ with $\rho < 1$, ensuring complementarity across task types and providing model collapse protection ($\alphaeff > \alphacrit$). By the CES Triple Role (\cite{sf:smirl2026a}, Theorem~7.1), the curvature parameter $K = (1-\rho)(J-1)/J$ simultaneously governs the superadditivity gap, the correlation robustness bonus, and the strategic manipulation penalty.
...
Grossman and Stiglitz~\cite{sf:grossman1980} prove that perfectly efficient markets are impossible: if prices fully reveal all information, no agent has incentive to pay for information, but then prices cannot incorporate information. The equilibrium involves partial revelation, with the degree of revelation depending on the cost of acquiring information.
...
\begin{equation}\label{sf:eq:gs_base}
...
\begin{equation}\label{sf:eq:cost}
...
\begin{equation}\label{sf:eq:efficiency}
...
\begin{equation}\label{sf:eq:eps_limit}
...
\begin{proof}
Two opposing forces act on $\lambda$. First, as $\phi$ increases, the number of independently informed trading strategies $n(\phi)$ rises (more mesh agents processing information independently). By the Holden-Subrahmanyam~\cite{sf:holden1992} result, competition among informed traders reduces $\lambda$: $\partial \lambda / \partial n < 0$. Second, as $\phi$ increases, noise trading volume $\sigma_u(\phi)$ decreases because human institutions---the primary source of noise trading---exit. Since $\lambda \propto 1/\sigma_u$, this increases $\lambda$: $\partial \lambda / \partial \sigma_u < 0$ and $\partial \sigma_u / \partial \phi < 0$, so $\partial \lambda / \partial \phi > 0$ through this channel.
...
This section applies the Lucas~\cite{sf:lucas1976} critique systematically to each monetary policy tool, identifying the specific friction each tool depends on and modeling that friction as a function of mesh participation $\phi$ and stablecoin ecosystem size $S$. The monetary macro structure follows Brunnermeier and Sannikov~\cite{sf:brunnermeier2014}.
...
\begin{definition}[Forward Guidance Effectiveness]\label{sf:def:fg}
...
\begin{proof}
The monotonicity in $\phi$ follows from equations~\eqref{sf:eq:fg} and~\eqref{sf:eq:qe}: both $\mathrm{FG}$ and the portfolio-balance component of $\mathrm{QE}$ are proportional to $(1-\phi)^{\alpha}$ for $\alpha > 0$, hence strictly decreasing. The monotonicity in $S$ follows from equation~\eqref{sf:eq:fr}: $\mathrm{FR}$ is decreasing in $S/\Scrit$ for $S \leq \Scrit$ and zero for $S \geq \Scrit$. The potential discontinuity arises because $\mathrm{FR}$ transitions from $\mathrm{FR}_0 \cdot (1 - S/\Scrit)^{\alpha_{FR}}$ to zero as $S$ crosses $\Scrit$; while technically continuous, the derivative $\partial \mathrm{FR}/\partial S$ is largest near $\Scrit$, producing a sharp decline in $\mathrm{MP}$ that is effectively discontinuous in practice.
\end{proof}

\subsection{The Brunnermeier-Sannikov Volatility Paradox}

Brunnermeier and Sannikov~\cite{sf:brunnermeier2014,sf:brunnermeier2016} identify a ``volatility paradox'': periods of low exogenous volatility can breed endogenous instability as financial institutions increase leverage in calm environments. The paradox applies directly to the mesh transition.
...
\begin{equation}\label{sf:eq:dol_capital}
...
\begin{definition}[Dollarization Reproduction Number]\label{sf:def:R0dollar}
The dollarization reproduction number is:
\begin{equation}\label{sf:eq:R0dollar}
...
% -------------------------------------------------------------------
% 6. THE TRIFFIN CONTRADICTION
% -------------------------------------------------------------------
\section{The Triffin Contradiction}

Triffin~\cite{sf:triffin1960} identified the fundamental tension in a reserve currency system: the world needs dollars for international transactions, but supplying those dollars requires the US to run deficits, which eventually undermines confidence in the dollar. Farhi and Maggiori~\cite{sf:farhi2018} formalize this as a model of the international monetary system with three zones for the reserve currency issuer. This section extends their framework with endogenous instability zone boundaries.
...
\begin{proposition}[Crisis Character Transition]\label{sf:prop:crisis}
...
\begin{equation}\label{sf:eq:crisis_prob}
...
\begin{enumerate}[label=(\roman*)]
\item The world demands dollar-denominated safe assets (Caballero, Farhi and Gourinchas~\cite{sf:caballero2017}). Stablecoins are a delivery mechanism for this demand.
...
\begin{corollary}[Triffin Squeeze]\label{sf:cor:triffin}
The ``Triffin squeeze'' is the condition:
\begin{equation}\label{sf:eq:triffin}
...
\item $S(t) \geq 0$: the stablecoin ecosystem size (measured in total stablecoin market capitalization or total user base).
\item $b(t) \geq 0$: the US Treasury debt-to-GDP ratio available as safe-asset backing.
\item $\eta(t) \in [0,1]$: financial sector capitalization as a fraction of total wealth, following Brunnermeier and Sannikov~\cite{sf:brunnermeier2014}. This measures financial system health: high $\eta$ corresponds to a well-capitalized financial sector; low $\eta$ corresponds to a crisis state.
\end{enumerate}
...
\emph{Low-mesh equilibrium.} At $\phi^L \approx 0$, the mesh participation equation~\eqref{sf:eq:phi_dot} has $\dot{\phi} = 0$ at $\phi = 0$ (trivially) and at any $\phi$ where $\mu_\phi(S,\eta) = r_\phi$. The Jacobian evaluated at $(\phi^L, S^L, b^L, \eta^L)$ has all eigenvalues with negative real parts when:
\begin{equation}
...
\begin{proposition}[Transition Condition]\label{sf:prop:transition}
...
\end{equation}
When $\Rsettle$ crosses 1, $\det(J)$ passes through zero, indicating a bifurcation. The transcritical structure follows from the logistic form of equation~\eqref{sf:eq:phi_dot}: the $\phi = 0$ equilibrium and the interior equilibrium collide at the bifurcation point, as in the standard epidemiological $R_0$ framework.
\end{proof}
...
\begin{proposition}[Endogenous Fiscal Discipline]\label{sf:prop:discipline}
In the high-mesh equilibrium, the yield spread on sovereign debt satisfies:
\begin{equation}\label{sf:eq:spread}
...
\textbf{Prediction 2: Forward guidance effectiveness decline.} The duration of market impact from central bank forward guidance decreases as mesh participation increases. The observable metric is the time from FOMC statement release to 90\% of the ultimate yield response. The model predicts this time declines from the current $\sim$2--4 weeks to less than 1 trading day as $\phi$ crosses 0.3 (approximately 30\% of Treasury market volume from autonomous agents). \emph{Timing:} 2029--2034. \emph{Evidence against:} forward guidance impact duration remaining stable or increasing as autonomous agent participation in Treasury markets grows above 20\%.
...
% -------------------------------------------------------------------
% 10b. PRELIMINARY EMPIRICAL EVIDENCE
% -------------------------------------------------------------------
...
\subsection{Prediction 4: Stablecoin--Treasury Yield Link}

Weekly data (2020--2026, $N = 319$) from DeFiLlama (stablecoin market capitalization) and FRED (Treasury yields, VIX, federal funds rate) test whether stablecoin growth predicts Treasury yield movements.\footnote{Scripts and replication data at \texttt{scripts/test\_stablecoin\_treasury.py}.}
...
The first-difference specification (changes in yield on changes in log market cap) provides cleaner identification: $\hat{\beta} = -0.300$ ($\text{SE} = 0.178$, $p = 0.092$). The negative sign indicates that stablecoin inflows are associated with \emph{lower} short-term Treasury yields, consistent with the mechanism: growing stablecoin reserves increase demand for T-bills, compressing yields at the short end. The effect is marginally significant.
...
At current scale (stablecoin market cap $\sim$\$306B versus \$6T in Treasury bills outstanding, a 5.1\% ratio), the effects remain small. The prediction is that the elasticity approximately doubles as stablecoin market cap approaches \$1 trillion.

\subsection{Prediction 2: Forward Guidance Absorption Speed}

Data from 213 FOMC meetings (2000--2026) test whether yield adjustment to FOMC statements has accelerated over time, using the ratio of the immediate (1-day) yield response to the 5-day cumulative response as a measure of absorption speed.\footnote{Scripts and replication data at \texttt{scripts/test\_fomc\_impact.py}.}
...
\subsection{Prediction 3: Dollarization Threshold Decline}

Cross-country panel data (18 countries, 2020--2024, $N = 77$) test whether the inflation rate at which stablecoin adoption accelerates has declined over time, consistent with the prediction that larger stablecoin ecosystems lower the threshold.\footnote{Scripts and replication data at \texttt{scripts/test\_inflation\_threshold.py}.}
...
% Thesis papers
\bibitem{sf:smirl2026a} Smirl, J. (2026a). The CES triple role: Superadditivity, correlation robustness, and strategic independence as three views of isoquant curvature. Chapter~2 of this thesis.

\bibitem{sf:smirl2026b} Smirl, J. (2026b). Complementary heterogeneity: A port-Hamiltonian theory of the AI transition. Chapter~3 of this thesis.

\bibitem{sf:smirl2026ed} Smirl, J. (2026). Endogenous decentralization: How concentrated capital investment finances the learning curves that enable distributed alternatives. Chapter~4 of this thesis.

\bibitem{sf:smirl2026mesh} Smirl, J. (2026). The mesh equilibrium: How heterogeneous specialized agents self-organize to exceed centralized provision after the crossing point. Chapter~5 of this thesis.

\bibitem{sf:smirl2026auto} Smirl, J. (2026). The autocatalytic mesh: Endogenous capability growth in self-organizing agent networks. Chapter~5, Part~II of this thesis.

\bibitem{sf:smirl2026mpg} Smirl, J. (2026). The monetary productivity gap. Chapter~7 of this thesis.

% Market microstructure
\bibitem{sf:grossman1980} Grossman, S.~J., \& Stiglitz, J.~E. (1980). On the impossibility of informationally efficient markets. \emph{American Economic Review}, 70(3), 393--408.

\bibitem{sf:kyle1985} Kyle, A.~S. (1985). Continuous auctions and insider trading. \emph{Econometrica}, 53(6), 1315--1335.

\bibitem{sf:holden1992} Holden, C.~W., \& Subrahmanyam, A. (1992). Long-lived private information and imperfect competition. \emph{Journal of Finance}, 47(1), 247--270.

\bibitem{sf:duffie2005} Duffie, D., G\^{a}rleanu, N., \& Pedersen, L.~H. (2005). Over-the-counter markets. \emph{Econometrica}, 73(6), 1815--1847.

\bibitem{sf:dou2025} Dou, W.~W., Goldstein, I., \& Ji, Y. (2025). AI-powered trading, algorithmic collusion, and price efficiency. NBER Working Paper No.\ 34054.

% Monetary economics
\bibitem{sf:brunnermeier2014} Brunnermeier, M.~K., \& Sannikov, Y. (2014). A macroeconomic model with a financial sector. \emph{American Economic Review}, 104(2), 379--421.

\bibitem{sf:brunnermeier2016} Brunnermeier, M.~K., \& Sannikov, Y. (2016). The I theory of money. Working paper, Princeton University.

\bibitem{sf:woodford2003} Woodford, M. (2003). \emph{Interest and Prices: Foundations of a Theory of Monetary Policy}. Princeton University Press.

\bibitem{sf:lucas1976} Lucas, R.~E. (1976). Econometric policy evaluation: A critique. \emph{Carnegie-Rochester Conference Series on Public Policy}, 1, 19--46.

% Dollarization and currency crises
\bibitem{sf:uribe1997} Uribe, M. (1997). Hysteresis in a simple model of currency substitution. \emph{Journal of Monetary Economics}, 40(1), 185--202.

\bibitem{sf:calvo1998} Calvo, G.~A. (1998). Capital flows and capital-market crises: The simple economics of sudden stops. \emph{Journal of Applied Economics}, 1(1), 35--54.

\bibitem{sf:obstfeld1996} Obstfeld, M. (1996). Models of currency crises with self-fulfilling features. \emph{European Economic Review}, 40(3--5), 1037--1047.

% International monetary system
\bibitem{sf:farhi2018} Farhi, E., \& Maggiori, M. (2018). A model of the international monetary system. \emph{Quarterly Journal of Economics}, 133(1), 295--355.

\bibitem{sf:caballero2017} Caballero, R.~J., Farhi, E., \& Gourinchas, P.-O. (2017). The safe assets shortage conundrum. \emph{Journal of Economic Perspectives}, 31(3), 29--46.

\bibitem{sf:triffin1960} Triffin, R. (1960). \emph{Gold and the Dollar Crisis: The Future of Convertibility}. Yale University Press.

% Stablecoin empirics
\bibitem{sf:ahmed2025} Ahmed, R., \& Aldasoro, I. (2025). Stablecoins and safe asset prices. BIS Working Paper No.\ 1270.

\bibitem{sf:gorton2022} Gorton, G.~B., Klee, E., Ross, C., Ross, S.~Y., \& Vardoulakis, A.~P. (2022). Leverage and stablecoin pegs. NBER Working Paper No.\ 30796.

\bibitem{sf:gorton2017} Gorton, G.~B. (2017). The history and economics of safe assets. \emph{Annual Review of Economics}, 9, 547--586.

% Diamond-Dybvig
\bibitem{sf:diamond1983} Diamond, D.~W., \& Dybvig, P.~H. (1983). Bank runs, deposit insurance, and liquidity. \emph{Journal of Political Economy}, 91(3), 401--419.

% Inequality
\bibitem{sf:piketty2014} Piketty, T. (2014). \emph{Capital in the Twenty-First Century}. Harvard University Press.

\bibitem{sf:jones2015} Jones, C.~I. (2015). Pareto and Piketty: The macroeconomics of top income and wealth inequality. \emph{Journal of Economic Perspectives}, 29(1), 29--46.

\bibitem{sf:gabaix2016} Gabaix, X., Lasry, J.-M., Lions, P.-L., \& Moll, B. (2016). The dynamics of inequality. \emph{Econometrica}, 84(6), 2071--2111.

% Network effects
\bibitem{sf:arthur1989} Arthur, W.~B. (1989). Competing technologies, increasing returns, and lock-in by historical events. \emph{Economic Journal}, 99(394), 116--131.

\bibitem{sf:arthur1994} Arthur, W.~B. (1994). \emph{Increasing Returns and Path Dependence in the Economy}. University of Michigan Press.

\bibitem{sf:katz1985} Katz, M.~L., \& Shapiro, C. (1985). Network externalities, competition, and compatibility. \emph{American Economic Review}, 75(3), 424--440.

% Growth theory (inherited)
\bibitem{sf:jones1995} Jones, C.~I. (1995). R\&D-based models of economic growth. \emph{Journal of Political Economy}, 103(4), 759--784.

\bibitem{sf:romer1990} Romer, P.~M. (1990). Endogenous technological change. \emph{Journal of Political Economy}, 98(5), S71--S102.

\bibitem{sf:bloom2020} Bloom, N., Jones, C.~I., Van Reenen, J., \& Webb, M. (2020). Are ideas getting harder to find? \emph{American Economic Review}, 110(4), 1104--1144.

\bibitem{sf:aghion2018} Aghion, P., Jones, B.~F., \& Jones, C.~I. (2018). Artificial intelligence and economic growth. In A.~Agrawal, J.~Gans, \& A.~Goldfarb (Eds.), \emph{The Economics of Artificial Intelligence} (pp.~237--282). University of Chicago Press.

% Baumol (inherited)
\bibitem{sf:baumol1967} Baumol, W.~J. (1967). Macroeconomics of unbalanced growth: The anatomy of urban crisis. \emph{American Economic Review}, 57(3), 415--426.

\bibitem{sf:lasry2007} Lasry, J.-M., \& Lions, P.-L. (2007). Mean field games. \emph{Japanese Journal of Mathematics}, 2(1), 229--260.
