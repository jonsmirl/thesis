...
**(c) Strategic independence:** $\Delta(S) \leq -\Omega(K_{\text{gen},S}) \cdot \text{deviation}^2$ (with coalition-specific curvature parameter $K_{\text{gen},S}$)
...
**Step 2 (Quantitative): Curvature comparison with $\kappa_{\min}$.** For general weights, the isoquant curvature varies across tangent directions. The normal curvature comparison for convex hypersurfaces (do Carmo, *Riemannian Geometry*, Prop. 3.1 and the comparison theorem for submanifolds of $\mathbb{R}^n$; see also Toponogov's theorem applied to sections of $\mathcal{I}_c$ by 2-planes through $\mathbf{x}^*$) gives: if $\kappa(\mathbf{v}) \geq \kappa_{\min} > 0$ for all tangent directions $\mathbf{v}$ at $\mathbf{x}^*$, then for points $\hat{\mathbf{x}}, \hat{\mathbf{y}} \in \mathcal{I}_1$ with geodesic distance $d = d_{\mathcal{I}}(\hat{\mathbf{x}}, \hat{\mathbf{y}})$ sufficiently small:

$$F(\alpha\hat{\mathbf{x}} + (1-\alpha)\hat{\mathbf{y}}) \;\geq\; 1 + \frac{\kappa_{\min}}{2}\,\alpha(1-\alpha)\,d^2 + O(d^4)$$

**Scope of the bound.** The comparison is local: it holds to second order in $d_{\mathcal{I}}$ for $\hat{\mathbf{x}}, \hat{\mathbf{y}}$ in a geodesic neighborhood of $\mathbf{x}^*/c$ on $\mathcal{I}_1$. For well-separated points, the qualitative bound (Step 1) already gives $F(\mathbf{x}+\mathbf{y}) > F(\mathbf{x}) + F(\mathbf{y})$ with a positive gap. The CES isoquant has strictly positive normal curvature at every interior point — at any $\mathbf{x} \in \mathbb{R}^J_{++}$ with $F(\mathbf{x}) = c$ and any tangent $\mathbf{v}$, we have $\mathbf{v}^T H_F \mathbf{v} = -(1-\rho)\sum_j (\partial_j F)\,v_j^2/x_j < 0$ — so a fully global quantitative bound exists using $\inf_{\mathbf{x}\in\mathcal{I}_c}\kappa_{\min}(\mathbf{x})$, which is positive on any compact subset of the interior. The local bound at $\mathbf{x}^*$ is sufficient for the theorem as stated and gives the tightest constant in the regime where inputs are close to the cost-minimizing bundle.
...
**Remark on proof strategy.** In the equal-weights case, $Y_2$ depended only on the idiosyncratic norm $\|\boldsymbol{\eta}\|^2$, which decoupled cleanly from the common mode. With general weights, $Y_2$ involves the weighted norm $\sum \epsilon_j^2/x_j^*$, which mixes the common and idiosyncratic components through the cross term above. Rather than constructing a weight-adjusted orthogonal decomposition (which would require working in the inner product $\langle u, v \rangle_W = \sum u_j v_j / x_j^*$ and would complicate the covariance structure), we proceed by computing $\text{Var}[Y_1]$ and $\text{Var}[Y_2]$ directly from the equicorrelation structure and then bounding $d_{\text{eff}}$ from below.
...
**Simplified bound (using $R_{\min}$):** The curvature bonus in the preceding expression involves the ratio $\Psi_2 J / \Psi_1^2$, which depends on the full spectrum of inverse effective shares. We need to show this is bounded below by a quantity involving only $K_{\text{gen}}$ (i.e., only $R_{\min}$).

The key inequality is: the idiosyncratic variance $\text{Var}[Y_2]^{\text{idio}}$ is bounded below by the contribution from the minimum-curvature direction alone. Formally, the $(J-1)$ constrained eigenvalues $\mu_1 \leq \cdots \leq \mu_{J-1}$ of $W|_T$ satisfy:

$$\Psi_2^{\text{tang}} \;:=\; \sum_{k=1}^{J-1}\mu_k^2 \;\geq\; (J-1)\,\mu_1^2 \;=\; (J-1)\,R_{\min}^2$$

(minimum squared $\leq$ mean of squares, applied in reverse: sum of squares $\geq$ count times minimum squared). Here $\Psi_2^{\text{tang}}$ is the sum of squared eigenvalues of the Hessian's action on the tangent space, which controls $\text{Var}[Y_2]^{\text{idio}}$ through the relation $\text{Var}[Q|_{\text{idio}}] = 2\tau^4(1-r)^2\,\Psi_2^{\text{tang}}$ (each independent idiosyncratic mode contributes variance proportional to $\mu_k^2$; under equicorrelation, the $(J-1)$ idiosyncratic modes have equal input variance $\tau^2(1-r)$, so the total is $2\sum_k \mu_k^2 \tau^4(1-r)^2$).

Substituting $(J-1)R_{\min}^2$ for $\Psi_2^{\text{tang}}$ and recalling $K_{\text{gen}} = (1-\rho)(J-1)\Phi^{1/\rho}R_{\min}/J$:

$$\text{Var}[Y_2]^{\text{idio}} \;\geq\; \frac{(1-\rho)^2 g^2 \Phi^{2/\rho}}{4c^2}\cdot 2\tau^4(1-r)^2\cdot(J-1)R_{\min}^2$$

Using the identity $(1-\rho)^2\Phi^{2/\rho}(J-1)R_{\min}^2 = K_{\text{gen}}^2 J^2/(J-1)$ (which follows directly from squaring the definition of $K_{\text{gen}}$ and multiplying by $(J-1)$):

$$= \frac{g^2 K_{\text{gen}}^2 J^2}{2(J-1)\,c^2}\cdot\tau^4(1-r)^2$$

At equal weights $g = 1/J$, so $g^2 J^2 = 1$, recovering the v2a expression $K^2\tau^4(1-r)^2/[2(J-1)c^2]$. $\checkmark$

The multi-channel $d_{\text{eff}}$ argument from v2a then yields (following the same Fisher information bridge, with $g^2 J^2$ replacing the factor of 1 that appeared at equal weights):
...
**Definition.** For a coalition $S \subseteq [J]$ with $|S| = k \geq 2$, define the **coalition curvature parameter**

$$K_{\text{gen},S} \;=\; (1-\rho)\cdot\frac{(k-1)\,\Phi^{1/\rho}\,R_{\min,S}}{k}$$

where $R_{\min,S}$ is the smallest root of the coalition secular equation $\sum_{j\in S}(w_j - \mu)^{-1} = 0$ with $w_j = 1/p_j = a_j^{-\sigma}$, i.e., the minimum Rayleigh quotient of $\text{diag}(1/p_j)_{j\in S}$ restricted to $\{\sum_{j\in S}v_j = 0\}$. For $|S| = 1$, set $K_{\text{gen},S} = 0$ (no internal redistribution possible).

**Properties of $K_{\text{gen},S}$:**

- $K_{\text{gen},S} > 0$ for all $|S| \geq 2$, all $\rho < 1$, all weight vectors. (Positive because $R_{\min,S} > \min_{j\in S} w_j > 0$.)

- At equal weights: $R_{\min,S} = J^{\sigma}$ and $\Phi^{1/\rho} = J^{-\sigma}$ for all $S$, so $K_{\text{gen},S} = (1-\rho)(k-1)/k$, which is independent of which $k$ components are in $S$ (by symmetry). In particular, $K_{\text{gen},[J]} = K$.

- In general, $K_{\text{gen},S} \neq K_{\text{gen}}$ restricted to $S$: the full-system minimum $R_{\min}$ and the coalition minimum $R_{\min,S}$ are computed over different constraint spaces. Neither inequality $R_{\min,S} \geq R_{\min}$ nor $R_{\min,S} \leq R_{\min}$ holds universally.

**Theorem C-G.** For all $\rho < 1$ and any coalition $S$ with $|S| = k \leq J/2$:

(i) *(Qualitative)* The manipulation gain satisfies $\Delta(S) \leq 0$.

(ii) *(Quantitative)* For any redistribution $\boldsymbol{\delta}_S$ with $\sum_{j\in S}\delta_j = 0$:

$$\Delta(S) \;\leq\; -\frac{K_{\text{gen},S}}{2k(k-1)c}\cdot\frac{k}{J}\cdot\frac{\|\boldsymbol{\delta}_S\|^2}{c} \;\leq\; 0$$

The bound tightens as $K_{\text{gen},S}$ increases (stronger complementarity or more balanced coalition weights).
...
**This establishes part (i), and it requires NO modification for general weights.**
...
From the Hessian at $\mathbf{x}^*$ (Section 3.2), for $\boldsymbol{\delta}$ with $\sum_{j\in S} \delta_j = 0$:

$$\boldsymbol{\delta}_S^T H_{SS}\boldsymbol{\delta}_S = -(1-\rho)\,g\sum_{j\in S}\frac{\delta_j^2}{x_j^*} = -\frac{(1-\rho)\,g\,\Phi^{1/\rho}}{c}\sum_{j\in S}\frac{\delta_j^2}{p_j}$$

The constrained Rayleigh quotient gives $\sum_{j\in S}\delta_j^2/p_j \geq R_{\min,S}\,\|\boldsymbol{\delta}_S\|^2$, so:

$$\boldsymbol{\delta}_S^T H_{SS}\boldsymbol{\delta}_S \;\leq\; -\frac{(1-\rho)\,g\,\Phi^{1/\rho}}{c}\cdot R_{\min,S}\cdot\|\boldsymbol{\delta}_S\|^2$$

Expressing this in terms of $K_{\text{gen},S}$:

$$\boldsymbol{\delta}_S^T H_{SS}\boldsymbol{\delta}_S \;\leq\; -\frac{K_{\text{gen},S}\,g\,k}{c(k-1)}\cdot\|\boldsymbol{\delta}_S\|^2$$
...
$$\Delta(S) \;\leq\; -\frac{K_{\text{gen},S}}{2k(k-1)c}\cdot\frac{k}{J}\cdot\frac{\|\boldsymbol{\delta}_S\|^2}{c}$$

$\blacksquare$

**Remark 1.** Part (c) is the most robust extension: the qualitative result (Shapley value in the core, manipulation gain $\leq 0$) holds for ALL weights without any curvature computation. Only the quantitative bound on the manipulation penalty requires $K_{\text{gen},S}$.

**Remark 2 (Comparison with the global $K_{\text{gen}}$).** Since neither $R_{\min,S} \geq R_{\min}$ nor $R_{\min,S} \leq R_{\min}$ holds in general, the coalition-specific parameter $K_{\text{gen},S}$ cannot be replaced by $K_{\text{gen}}$ without additional assumptions. A universally valid (but potentially looser) bound uses $\min_{j \in S} 1/p_j$ in place of $R_{\min,S}$:

$$\Delta(S) \;\leq\; -\frac{(1-\rho)\,g\,\Phi^{1/\rho}}{2c\,p_{\max}^{(S)}}\cdot\frac{k}{J}\cdot\frac{\|\boldsymbol{\delta}_S\|^2}{c}$$

where $p_{\max}^{(S)} = \max_{j\in S}p_j$. At equal weights both bounds coincide with $K$ since all $p_j$ and all $R_{\min,S}$ are equal.
...
**(c) Strategic independence.** For any coalition $S$ with $|S| = k \geq 2$, define $K_{\text{gen},S} = (1-\rho)(k-1)\Phi^{1/\rho}R_{\min,S}/k$ where $R_{\min,S}$ is the smallest root of $\sum_{j\in S}(w_j - \mu)^{-1} = 0$. Then the manipulation gain for any redistribution $\boldsymbol{\delta}_S$ with $\sum_{j\in S}\delta_j = 0$ satisfies:

$$\Delta(S) \;\leq\; -\frac{K_{\text{gen},S}}{2k(k-1)c}\cdot\frac{k}{J}\cdot\frac{\|\boldsymbol{\delta}_S\|^2}{c} = -\Omega(K_{\text{gen},S})\cdot\text{deviation}^2$$

At equal weights, $K_{\text{gen},S} = (1-\rho)(k-1)/k$ for all coalitions $S$ of size $k$.
...
No clean closed-form perturbation expansion exists for general $J$ and $\rho$. The leading correction to $K_{\text{gen}}$ is second order in $\boldsymbol{\epsilon}$ (by the symmetry of $K_{\text{gen}}$ under permutations, there is no first-order term), and its sign depends on $\rho$: numerical computation shows that $K_{\text{gen}} < K$ for $\rho < 0$ and $K_{\text{gen}} > K$ for $\rho$ sufficiently close to 1, with a crossover near the Cobb-Douglas limit $\rho = 0$ (see §12.4). For practical purposes, the algorithm in §10.1 computes $K_{\text{gen}}$ to machine precision for any specific $(\rho, \mathbf{a})$ pair.
