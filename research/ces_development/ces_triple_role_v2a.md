...
Under Shapley allocation, the coalition's value changes by at most $(k/J)\cdot\Delta F$ (by efficiency). The complementarity premium $\frac{K k(J-k)}{2J(J-1)c}\delta^2$ is already priced into the efficient allocation. The CES game is convex ($F$ concave implies the characteristic function $v(S) = \max_{\mathbf{x}_S} F(\mathbf{x}_S, \mathbf{0}_{-S})$ is convex; see Shapley 1971, "Cores of Convex Games"), so the Shapley value lies in the core, and core allocations satisfy the first-order conditions at the efficient point. No deviation is profitable.
